...
<!-- Boxplots offer provide a way of visualising the __interquartile range (IQR).__ You can see what each part of the boxplot represents in Figure @fig-boxplotdesc.   -->
...
## Bivariate associations (two variables)
...
We can use a violin plot to visualise the association between one numeric variable and one categorical variable - these will be our y- and x-axis values respectively.
It combines a summary of the data's range and a kernel density estimation, providing a detailed view of the distribution. 
...
Note that [boxplots don't do a good job of representing how variables are actually distributed](https://nightingaledvs.com/ive-stopped-using-box-plots-should-you/){target="_blank"}, so we don't recommend including a boxplot in your actual written reports.
Instead, use a violin plot with jittered points and group means (above).
...
# Add more variables using facets
