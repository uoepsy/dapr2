...
When we specify the linear model in `R`, we include after the tilde sign ($\sim$), the variables that appear to the right of the $\beta$s. The intercept, or $\beta_0$, is a constant. That is, we could write it as multiplied by 1.
...
To fit the model in `R` we use the `lm()` function. 

We give `lm()` two arguments:

1. the model formula, using the format `outcome ~ predictor`, or `dependent_variable ~ independent_variable`.
1. the dataset that the outcome and predictor variables live in.

For example, here's the general template for using `lm()`:
...
Now, the following code chunk fits the model with outcome `recall_accuracy`, predictor `age`, and dataset `recalldata`.
Then it stores the linear model in a variable called `recall_simp`.
...
- An individual aged 0 years was expected to have a recall accuracy of $`r round(coef(recall_simp)[1],2)`$ percent.  
...
- Increasing one year in age is associated with a decrease of 0.3 percent in recall accuracy.
(Why percent? Because the outcome variable is a percentage.)
...
> The effect of age is not statistically significant ($b$ = –0.3, 95% CI [–0.78, 0.17], $p$ = .20), so we fail to reject the H0.
