...
#### Significance tests for the intercept in regular linear regression vs. logistic regression

When the outcome scale is log-odds, it's actually a lot more interesting if the intercept is significantly different from zero!
...
> The effect of WAIS is statistically significant ($b$ = –0.32, 95% CI [–0.58, –0.12], $p$ = .005), so we can reject H0.
...
- `focal.predictors`: the predictor we want on the x axis.
...
## Predicting probabilities for specific predictor values

What is the probability that somebody with a WAIS score of 15 shows senility symptoms?

To start, take the model's mathematical expression and substitute in the model's coefficients:

$$
\begin{align}
\text{logodds(senile)}_\text{WAIS=15} &= \beta_0 + (\beta_1 \cdot \text{WAIS}) \\
&= 2.40 + (-0.32 \cdot \text{WAIS}) \\
\end{align}
$$
...
$$
\begin{align}
\text{logodds(senile)}_\text{WAIS=15} &= 2.40 + (-0.32 \cdot \text{WAIS}) \\
 &= 2.40 + (-0.32 \cdot 15) \\
 &= 2.40 + (-4.80) \\
 &= -2.40 \\
\end{align}
...
So, the predicted probability that somebody will show symptoms of senility if their WAIS score is 15 is 8.3%.
