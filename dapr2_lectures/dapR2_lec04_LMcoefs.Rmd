...
+ **Research questions** are statements of what we intend to study and define:
...
> **Does increased study time improved test scores in school age children?**

--

+ **Statistical hypotheses** are testable mathematical statement.
...
# Defining null
+ Conceptually:
	+ If $x$ yields no information on $y$, then $b_1 = 0$
...
+ We have already seen how we calculate $b_1$.
+ The associated test statistic to for beta coefficients is a $t$-statistic

$$t = \frac{b}{SE(b)}$$

+ where
  + $b$ = any beta coefficient we have calculated
...
$$SE(b) = \sqrt{\frac{\frac{SSE}{(N-k-1)}}{\sum(x_i - \bar{X})^2}}$$
...
+ From this formula, we can see that the SEs for regression slopes will be smaller when:
	+ Residual variance ($SSE$) is smaller
...
+ So all we have left is SSE.
+ From last our linear model introduction recordings:
...
---
# Back to our example (3)

$$SE(b) = \sqrt{\frac{\frac{SSE}{(N-k-1)}}{\sum(x_i - \bar{X})^2}} = \sqrt{\frac{\frac{21.16364}{(10-1-1)}}{20.625}} = \sqrt{\frac{2.65455}{20.625}} = 0.35814$$

+ and finally

$$t = \frac{b}{SE(b)} = \frac{1.055}{0.35814} = 2.945773 = 2.95$$
...
--
+ Now we need our critical value to compare our $t$-value to.
...
+ We can also compute confidence intervals for $b_1$
