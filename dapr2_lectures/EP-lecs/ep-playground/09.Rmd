...
```{r}
set.seed(1)
main_sample <- sort(sample(1:100, size = 50))
matrix(main_sample, ncol = 5)
```
...
for(i in 1:n_boot_samples){
  boot_samp <- sample(main_sample, size = boot_sample_size, replace = TRUE)
  boot_means <- c(boot_means, mean(boot_samp))
}
