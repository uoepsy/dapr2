...
## TODO Welcome-back warm-up on Wooclap
...
To notice that **sometimes the line's intercept is far away from the range of data we observe.**

[show again same plots]

In such cases, sometimes it doesn't even make sense for a predictor to *have* a value of zero!

<!-- - Imagine analysing a group's `shoe size ~ height in m`. A height of zero metres is nonsense. -->

## A new tool in our toolkit: Mean-centering predictors

[analysis plan with "transform continuous predictors" indicated as WE ARE HERE]
...
Subtracting any amount from a bunch of data will shift the data around correspondingly.

[plots of data with 1 subtracted, 2, 5, etc]


If we subtract the _mean_, then what used to be the mean of the data will now be zero.
(Imagine the mean is 5. If a data point is equal to 5, and we subtract the mean of 5, then now that data point is equal to 0.)
...
outdoors1 |>
  ggplot(aes(x = outdoor_time, y = wellbeing)) +
  geom_point(size = 8) +
  geom_abline(aes(intercept = m_outdoors1$coefficients[['(Intercept)']], slope = m_outdoors1$coefficients[['outdoor_time']]), colour = dapr2red, linewidth = 2) +
  ylim(10, 40) +
  xlim(-7, 7) +
  NULL
```
...
outdoors1 |>
  ggplot(aes(x = outdoor_time_c, y = wellbeing)) +
  geom_point(size = 8) +
  geom_abline(aes(intercept = m_outdoors2$coefficients[['(Intercept)']], slope = m_outdoors2$coefficients[['outdoor_time_c']]), colour = dapr2red, linewidth = 2) +
  ylim(10, 40) +
  xlim(-7, 7)
```
:::
...
## TODO Vote: Which data is mean-centered?
...
## TODO If this seems a little familiar...

Flashback to [standardising predictors in week 2 lecture](https://uoepsy.github.io/dapr2/2526/lectures/dapr2_02_lm2.html#/option-2-standardising-the-variables)

[venn diagram of mean-centering and z-scoring]

[L: plot of data mean-centered]

[R: plot of data z-scored]



# Modelling with a mean-centered predictor

## TODO Mean-centre a predictor in R
...
<!-- NB: Not teaching `scale()` for centering because scale() can produce funny behaviour down the road. -->



## TODO Fit the model after mean-centering
...
## TODO Intepreting the new coefficients after centering

<!-- https://stefanocoretta.github.io/sqmb/slides/w09/#53 -->
...
- When `x` doesn't have a meaningful/interpretable zero point.
  - e.g., if modelling `shoe size ~ weight`, a weight of zero is nonsense.
- When you want the model's intercept to represent the average outcome at the mean value of `x`.
...
- When `x` has a meaningful/interpretable zero point.
  - e.g., if modelling `skill ~ years of training`, it makes sense to think about someone with zero years of training.
- When you want the model's intercept to represent the average outcome for whatever `x = 0` already represents.
...
## Fit the model (now outdoor time is mean-centered)
...
- When `outdoor_time = 0` and `envt = 0`, the average `wellbeing` is about 57.
  - !!! **now `outdoor_time = 0` means at the mean of time spent outdoors** !!!
  - `envt = 0` means `envt = nature` (the reference level).
...
- When `outdoor_time = 0` = **at the mean of outdoor time** (!!!), being in an urban environment is associated with a decrease in wellbeing of about 26 points.
:::
::::
...
# But: These models are not quite right for this data

## These models are not quite right for this data
...
1. Both versions of the model assume that **whatever your environment,** an extra hour of being outdoors improves your wellbeing by about 8.

2. Both versions of the model assume that **whatever your time spent outdoors,** being in an urban environment reduces your wellbeing by about 26 points.
...
## TODO "Depends on" $\rightarrow$ Interactions
...
- When `envt = natural`, the association is about 12 (the `outdoor_time` coef above).
...
How will the meaning of each coefficient change when `outdoor_time` is mean-centered?

<!-- FIXME: In future years, prediction activity here would be nice. -->
...
## TODO could do visual intuitions here again
...
:::{.dapr2callout style="font-size:250%"}
...
# Where do interactions fit in the analysis plan from last term?
