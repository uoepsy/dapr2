...
In such cases, sometimes it doesn't even make sense to for a predictor to *have* a value of zero!

<!-- - Imagine analysing a group's `shoe size ~ height in m`. A height of zero metres is nonsense. -->

In such cases, we can *transform* our variable so that the value zero becomes more meaningful in the context of that variable.
...
![](figs/TODO-venn-meancentre-zscore.jpeg){fig-align="center"}

Mean-centered variables still have their original standard deviation.
...
- When `outdoor_time = 0`, so **when outdoor time is at its mean,**, being in an urban environment is associated with a decrease in wellbeing of about 26 points.
:::
::::
