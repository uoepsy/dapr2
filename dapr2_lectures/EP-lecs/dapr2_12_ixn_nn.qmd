...
## Before we fit the model, remember our key points about multiple regression/interactions
...
Now the x axis shows hours of sunlight, and each panel shows a group of `outdoor_time`s, binned/chunked together.

<!-- We've flipped the interacting variables, but the pattern of the interaction is still there. -->

This plot shows that **the association between sunlight hours and wellbeing is different, when different amounts of time are spent outdoors.**
...
We have all the information we need to figure out the simple slopes:
...
**Smush together the numbers that are multiplied with $\text{outdoor_time}$. (In math terms, we're factoring out the variable $\text{outdoor_time}$ from both terms that contain it.)**
...
**Add up those smushed-together numbers and we're done!**
...
- When `outdoor_time = 0` (mean hrs outdoors) and `hrs_sun = 0` (mean sunlight hours), the estimated average `wellbeing` is 104.8.
...
- Specifically when `outdoor_time = 0` (at the mean number of hours spent outdoors), the sun shining for an extra hour is associated with an increase in wellbeing of 14.4 points.
...
- So, the association of `hrs_sun_c` with `wellbeing` when ...
  - `out_t_c = 0` is 14.4
  - `out_t_c = 1` is 14.4 + 3.3 = 17.7
  - `out_t_c = 2` is 14.4 + 3.3 + 3.3 = 21.0
  - ...
