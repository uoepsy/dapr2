...
class: center, middle
# Questions...
**Next we will look at how we interpret the hospital model in full with effects coding**
...
```{r, eval = FALSE}
lm(SWB ~ Treatment + Hospital + Treatment*Hospital, data = hosp_tbl)
```
...
m2 <- lm(SWB ~ Treatment + Hospital + Treatment*Hospital, data = hosp_tbl)
```
