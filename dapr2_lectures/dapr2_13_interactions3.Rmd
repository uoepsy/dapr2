...
salary3 <- tibble(
  ...
  location = factor(location, labels = c("Edinburgh", "Inverness")),
  serv = serv
)
...
+ Suppose our company also had two sites, Edinburgh and Inverness, and we wanted to see if the salaries across department differed depending on location.
...
colnames(eg1) <- c("Edinburgh","Inverness")
...
```{r,, echo=FALSE}
dat1 <- tibble(
  location = c("Edinburgh", "Edinburgh", "Inverness", "Inverness"),
...
colnames(eg2) <- c("Edinburgh","Inverness")
...
colnames(eg3) <- c("Edinburgh","Inverness")
...
+ The difference in salary between Accounts in Edinburgh and Inverness
...
+ The difference between salary in Accounts and Store managers between Edinburgh and Inverness
...
colnames(ex1_means) <- c("Edinburgh","Inverness")
...
+ The difference in salary between Accounts in Edinburgh and Inverness is £1,980. The salary is lower in Inverness. 
...
+ The difference between salary for Accounts and Store managers between Edinburgh and Inverness, differs by £8,420. The difference is greater in Inverness than in Edinburgh.
...
+ Consider our salary example:
  + We could ask does the change in difference in the effect of years of service across departments, differ dependent on whether you are based in Edinburgh or Inverness.
  
+ Note we have three predictors here:
  + Years of service (continuous)
  + department (binary; Store Manager, Accounts)
  + location (binary; Edinburgh, Inverness)
