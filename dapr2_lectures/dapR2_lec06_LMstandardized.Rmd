...
$$b_1 = \hat \beta_1 \frac{s_x}{s_y}$$

+ where;
  + $b_1$ = standardized beta coefficient
...
+ $b_0$ (intercept) = zero when all variables are standardised:
$$
...
---
# Relationship to r
+ Standardized beta's ( $b_1$ ) = correlation coefficient ( $r$ ) for a linear model with a single continuous predictor.

$$b_1 = \hat \beta_1 \frac{s_x}{s_y} = \left( r_{x,y} \frac{s_y}{s_x} \right) \frac{s_x}{s_y}= r_{x,y}$$

+ In our example, the $b_{hours}$ = 0.72
