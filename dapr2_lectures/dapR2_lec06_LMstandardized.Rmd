...
+ Standardization of coefficients
  ...
  + Standardized $b$ and correlation coefficient ($r$)

---
# Unstandardized vs standardized coefficients
- So far we have calculated unstandardized $b$.
...
+ **If we want to compare the effects of variables on different scales**
  ...
  + Note, this is exactly the same argument as we made in dapR1 concerning use of effect sizes.


---
# Standardizing the coefficients
+ After calculating a $b$, it can be standardized by:


$$\beta_1 = b_1\frac{SD_x}{SD_y}$$

+ where;
  + $\beta_1$ = standardized beta coefficient
  + $b_1$ = unstandardized beta coefficient
...
$$Z_{xi} = \frac{x_i - \bar{x}}{SD_x}$$

+ and the $z$-score for $y$:

$$Z_{yi} = \frac{y_i - \bar{y}}{SD_y}$$

+ That is, we divide the individual deviations from the mean by the $SD$
...
+ $b_0$ (intercept) = zero when all slopes are standardised.
...
>**For every standard deviaton increase in hours of study, test score increases by 0.72 standard deviations**

---
# Relationship to r
+ Standardized beta's ($\beta$) = correlation coefficient ($r$) for a linear model with a single continuous predictor.

+ In our example, the $\beta_{hours}$ = 0.72
...
+ $\beta$ is a standardized measure of the linear slope.
