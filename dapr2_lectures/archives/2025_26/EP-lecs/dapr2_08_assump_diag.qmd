...
No single threshold for outliers will work for cm AND log units AND yards AND millions of pounds AND ...
:::
...
p_infl + labs(title=element_blank(), y = 'y') 
```
...
```{r message = FALSE, warning=F, echo=F, fig.align='center', fig.width = 7}
p_good + labs(title=element_blank())
```
:::
...
```{r fig sens, message = FALSE, warning=F, echo=F}
set.seed(221020)
...
p_with <- df_sens |>
  ...
  labs(y = 'y', title = 'With unusual point') +
  ylim(-5, 20)
...
p_without <- df_sens |>
  ...
  geom_point(size = 5) +
  geom_smooth(method = 'lm', se = F, linewidth = 2) +
  labs(y = 'y', title = 'Unusual point removed') +
  ylim(-5, 20)
```
