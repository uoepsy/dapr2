...
```{r}
quantile(sampling_distrib$sample_means)
```

```{r}
quantile(
  sampling_distrib$sample_means,
  probs = c(0.025, 0.975)
)
```
...
Before, we got the sampling distribution of sample MEANS.

But we could also get, for example, the sampling distribution of sample MEDIANS.
