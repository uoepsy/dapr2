...
+ Traditionally coded with a 0 and 1
  ...
  + We will come back to this in more detail in multiple regression when we extend to categorical variables with 2+ levels
...
$$score_i = b_0 + b_1hours_{1i} + \epsilon_i$$
...
$$score_i = b_0 + b_1study_{1i} + \epsilon_i$$
...
.pull-left[
+ As before, the intercept is the expected value of $y$ when $x$=0
...
---
# Interpretation
+ $b_0$ = expected value of $Y$ when $X$ = 0
  + Or, the mean of group coded 0 (those who study alone)
  
+ $b_1$ = difference between the means of the two groups.
...
$$\hat{score} = b_0 + b_1study_{1i}$$

+ For those who study alone ( $study=0$ ):

$$\hat{score_{alone}} = b_0 + b_1*0$$

+ So;

$$\hat{score_{alone}} = b_0$$



---
#  Equations for each group 
+ What would our linear model look like if we added the values for $study$.

+ For those who study alone ( $study = 1$ ):

$$\hat{score_{others}} = b_0 + b_1*1$$

+ So;

$$\hat{score_{others}} = b_0+ b_1$$

+ And if we re-arrange;

$$b_1 = \hat{score_{others}} - b_0$$
...
bin +
  ...
  geom_text(x=1.55, y = 5.5, label = "b1", size=3, col = "red") +
  geom_text(x=0.65, y = gpM[[1,2]] , label = "b0", size=3, col = "red")
...
+ And we assess the significance of preditors in the same way

+ We use the standard error of the coefficient to construct:
  + We calculate the $b$ = difference between groups
...
--- 
# Standardizing b
+ When discussing continuous predictors we discussed standardized $\beta$ and unstandardized $b$

+ Recall, when we calculated $\beta$ we used $SD_x$.
...
+ As such, we do not standardized 
