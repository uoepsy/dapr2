---
title: "Mean-centering and numeric/categorical interactions"
...
:::hcenter
:::woo
https://app.wooclap.com/events/RADCCXP/
:::
:::
...
d1 |>
  ggplot(aes(x = x, y = y)) +
  geom_point(size = 3) +
  geom_smooth(se = F, method = 'lm', linewidth = 2.5) +
...
... then the line's intercept represents **the UK shoe size we'd expect for someone with average height: size 7.**
...
![](figs/workflow-09.svg){fig-align="center"}
...
- Mean-centered variables and standardised/z-scored variables _both_ have a mean of 0.

- But mean-centered variables still have their original standard deviation and their original units.
...
outdoors1 |>
  ggplot(aes(x = outdoor_time, y = wellbeing)) +
  geom_point(size = 8) +
  geom_abline(aes(intercept = m_outdoors1$coefficients[['(Intercept)']], slope = m_outdoors1$coefficients[['outdoor_time']]), colour = '#3366ff', linewidth = 3) +
  ylim(10, 40) +
  xlim(-7, 7) +
  NULL
```
...
outdoors1 |>
  ggplot(aes(x = outdoor_time_c, y = wellbeing)) +
  geom_point(size = 8) +
  geom_abline(aes(intercept = m_outdoors2$coefficients[['(Intercept)']], slope = m_outdoors2$coefficients[['outdoor_time_c']]), colour = '#3366ff', linewidth = 3) +
  ylim(10, 40) +
  xlim(-7, 7)
```
:::
...
In `mutate()`, subtract the mean of a variable from each observation of that variable.
...
I write `_c` after the variable name to represent that it's been centered.
...
- When centered outdoor time is equal to zero, **i.e., when outdoor time is at its mean value,** the average estimated wellbeing is 29.94 points.


**`outdoor_time_c`**

- When centered outdoor time increases by one hour, wellbeing increases by 2.73 points.
- So: 
  - one hour above the mean: 29.94 + 2.73 = 32.67
  - two hours above the mean: 29.94 + 2.73 + 2.7 = 35.4
  - ...
...
- When `outdoor_time = 0` and `envt = 0`, average estimated `wellbeing` is 19.05 points.
...
- Holding `envt` constant (that is, in either `nature` or `urban` environments), spending an additional hour outdoors is associated with an increase in wellbeing of 7.78 points.


**Environment:**

- Holding `outdoor_time` constant (that is, for any value of `outdoor_time`), being in an urban environment is associated with a decrease in wellbeing of 26.25 points.
:::
::::
...
## Recap: Two core ideas for multiple regression
...
Each predictor's slope represents the association of the given predictor with the outcome **when every other predictor is held constant.**
...
- When `outdoor_time_c = 0` and `envt = 0`, the estimated average `wellbeing` is 56.90 points.
  - **`outdoor_time_c = 0` represents the mean of time spent outdoors**
  - `envt = 0` means `envt = nature` (the reference level).
...
1. This model assumes that **whatever your environment,** an extra hour of being outdoors improves your wellbeing by 7.78 points.

<br>

2. This model assumes that **whatever your time spent outdoors,** being in an urban environment reduces your wellbeing by 26.25 points.
...
```{r echo=F, fig.dim=c(10, 7), fig.align="center"}
plot_data_probe +
  ylim(18, 95) +
  theme(text = element_text(size=28)) +
  NULL
...
:::{.dapr2callout .fragment}
`(Intercept)`: When all predictors are zero, the average estimated wellbeing is –3.39 points.
...
:::{.dapr2callout .fragment}
`outdoor_time`: Spending an additional hour outdoors is associated with an increase in wellbeing of 12.29 points **when `envt = 0 = nature` [*not* "holding `envt` constant"].**
:::

:::{.dapr2callout .fragment}
`envturban`: Being in an urban environment is associated with an increase in wellbeing of 20.29 points **when `outdoor_time = 0` hours [*not* "holding `outdoor_time` constant"].**
:::


:::{.dapr2callout .fragment}
`outdoor_time:envturban`: When we change environment from `natural` to `urban`, the **association between `outdoor_time` and `wellbeing` decreases** by 9.56.

<!-- - When `envt = natural`, the association is the one estimated above, 12 (the `outdoor_time` coef above). -->
...
## A visual intuition: `Intercept` {visibility="uncounted"}
...
## A visual intuition: `outdoor_time`  {visibility="uncounted"}
...
p_m3_int +
  ...
  geom_text(
    x = 1.2, y = 0, label = '12.29 wellbeing increase', colour = 'black', size = 6, hjust = 0
  ) +
  NULL
...
:::{.dapr2callout}
`outdoor_time`: **Only** when `envt = 0 = nature`, spending an additional hour outdoors is associated with an increase in wellbeing of 12.29 points.
:::

## A visual intuition: `envturban`  {visibility="uncounted"}
...
p_m3_int +
  ...
  geom_text(
    x = 0.3, y = 6, label = '20.29 wellbeing increase', colour = 'black', size = 6, hjust = 0
  ) +
  NULL
```

:::{.dapr2callout}
`envturban`: **Only** when `outdoor_time = 0` hours spent outdoors, being in an urban environment is associated with an increase in wellbeing of 20.29 points.
:::



## A visual intuition: `outdoor_time:envturban`  {visibility="uncounted"}
...
To get the slope for the `urban` line, we add up the slope for `natural` and the interaction coefficient: <br> 12.29 + (–9.56) = 2.73.
:::
...
## Interpreting coefs with centering, interactions
...
:::{.dapr2callout .fragment}
`(Intercept)`: When all predictors are zero, the average estimated wellbeing is 56.45 points.
...
:::{.dapr2callout .fragment}
`outdoor_time_c`: **Only** when `envt = 0 = nature`, spending an additional hour outdoors is associated with an increase in wellbeing of 12.29 points.
:::

:::{.dapr2callout .fragment}
`envturban`: **Only** when `outdoor_time_c = 0`, **which represents the average number of hours spent outdoors,** being in an urban environment is associated with a decrease in wellbeing of 26.26 points.
:::

:::{.dapr2callout .fragment}
`outdoor_time_c:envturban`: When we change environment from `natural` to `urban`, the **association** between `outdoor_time_c` and `wellbeing` decreases by 9.56 points.

- When `envt = natural`, the association is 12.29 (the `outdoor_time_c` coef above).
- When `envt = urban`, the association is 12.29 – 9.56 = 2.73.
:::
...
## Three core ideas for interaction models

<br>

:::{.dapr2callout style="font-size:120%;"}

A model's intercept is the mean outcome **when every predictor in the model is at zero.**
(Same as always. This interpretation never changes.)
...
- In interaction models, it's *not* correct to say "when every other predictor is held constant"—the slope estimate **only** applies when other predictors = 0.
- That's why, in interaction models, we call slopes "conditional effects": slope estimates are **conditional on all other predictors being 0.**
...
The interaction term represents how much one predictor's slope is **adjusted** when the other predictor's value increases by 1.
...
## Building an analysis workflow {visibility="uncounted"}
