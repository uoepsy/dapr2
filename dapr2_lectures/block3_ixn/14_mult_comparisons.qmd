...
The model in R syntax: `SWB ~ Treatment * Hospital`

<br>

For the mathematical model formulation:

- How many predictors do we need to represent `Treatment`?
- How many predictors do we need to represent `Hospital`?
- How many interaction terms do we need?

:::hcenter
:::woo
https://app.wooclap.com/events/IDEPDLG/
:::
:::
...
## Retrieval practice: Simple effects
...
Now that we have categorical predictors, the best way to test simple slopes is using the R code below.

(Don't worry too much about what the function `emmeans()` is doing, just trust that it works.)
...
**What's the probability that we'll roll a 1 at least once in $m$ rolls?**


:::hcenter
:::woo
https://app.wooclap.com/events/IQEPDLG/
:::
:::
...
## The probability of rolling 1 as a function of $m$
...
tibble(
  ...
  geom_point(size = 3) +
  labs(
    y = 'Prob of rolling at least one 1',
    x = 'm\n(number of rolls)'
  )
```
...
We're essentially setting the probability of incorrectly rejecting H0 **at least once in this set of tests** to .05.
...
## Another common correction: <br> Tukey's Honest Significant Differences (HSD)
...
- When you want to compare the means of levels of a categorical predictor (which is what we're doing, when we're testing simple effects!).
- When you're interested in many/all pairwise comparisons between groups.

<br>

<!-- Check the [flashcards](https://uoepsy.github.io/dapr2/2627/flashcards/){target="_blank"} for more details. -->




# When do we need to correct for multiple comparisons?

## When do we need to correct for multiple comparisons?

<!-- This is a question that even statisticians disagree about. -->

<!-- <br> -->

**Whenever you are using more than one test to draw conclusions.**

For example, this could be:

- testing multiple simple effects (like we saw before)
- running two linear models
- running one linear model and one t-test
- running one linear model and one chi-squared test
- ...


**My advice to avoid the multiple comparisons problem:**
...
- Only run the statistical tests that are **absolutely necessary** for addressing your research question(s).
  - This also helps avoid [Questionable Research Practices like p-hacking and HARKing](https://www.psychiatrist.com/jcp/harking-cherry-picking-p-hacking-fishing-expeditions-and-data-dredging-and-mining-as-questionable-research-practices/){target="_blank"}.
...
- Not just when you're testing multiple simple effects.
- Any time you're running and drawing conclusions based on more than one statistical test.
:::
