...
## Retrieval practice: Specify an interaction model

<br>

:::hcenter
:::woo
https://app.wooclap.com/events/AQHUOUJ/
:::
:::
...
## Guess the meanings of interaction coefs
...
**Without looking ahead, try to guess: What will each coefficient mean?**


<br>

Think to yourself or chat with a neighbour. Then:
...
- When `mm_rain = 0`, spending an additional hour outdoors is associated with an increase in wellbeing of 5.33 points.
...
- When `outdoor_time = 0`, an additional mm of rain is associated with a decrease in wellbeing of 0.08 points.
...
probe_interaction(
  m1,
  pred = outdoor_time,  # the predictor on the x axis
  modx = mm_rain,       # the other predictor
  interval = TRUE       # do we want 95% confidence intervals (yes we do)
)$interactplot
```
...
## Guess the meanings of mean-centered interaction coefficients
...
- Specifically when `mm_rain = 0` (at the mean rainfall), spending an additional hour outdoors is associated with an increase in wellbeing of 2.00 points.
...
- Specifically when `outdoor_time = 0` (at the mean outdoor time), an additional mm of rain is associated with a decrease in wellbeing of 0.26 points.
...
- Increasing centered rainfall by 1 mm decreases the association of `outdoor_time_c` with `wellbeing` by 0.03 points.
...
- **Equivalently:** Increasing centered outdoor time by 1 decreases the association of `mm_rain_c` with `wellbeing` by 3.3.
