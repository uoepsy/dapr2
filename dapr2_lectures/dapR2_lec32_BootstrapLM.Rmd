...
.pull-left[
```{r}
rep_1 <- tib1[sample(nrow(tib1), 10, replace = T),]
rep_1
```
]
...
.pull-left[
```{r}
rep_2 <- tib1[sample(nrow(tib1), 10, replace = T),]
rep_2
```
]
...
.pull-left[
```{r}
rep_3 <- tib1[sample(nrow(tib1), 10, replace = T),]
rep_3
```
]
...
- For every centimetre increase in height, there is a 1.05kg increase in weight. The 95% CI [50.9, 183.9] does not include 0, and as such we can reject the null at $\alpha = 0.05$
