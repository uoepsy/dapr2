...
- subtracting the mean of `score` ( $\bar{y}$ ) - which is the same for everyone
- squaring each of these values
- and summing them

We will build an object called `ss_tab` stand for sums of squares table. Here we will add our calculations to the original data.
...
- Taking the observed values of `score` for each individual ( $y_i$ )
- subtracting the predicted value of `score` for each individual from our model ( $\hat{y}_i$ )
- squaring each of these values
- and summing them
...
- subtracting the mean of `score` ( $\bar{y}$ )
- squaring each of these values
- and summing them
...
If we wanted to save this specific value from the summary output, we can:
```{r}
