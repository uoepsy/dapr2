...
+ High leverage cases
	+ Cases with an unusual combination of predictor ( $x_i$ ) values
...
#  Leverage 
.pull-left[
+ High leverage cases are those with an unusual predictor or combination of predictor values
	+ In simple linear model: an $x$ value far from the $\bar{x}$ 

+ **Why are we interested in leverage?** 
+ High leverage cases have potential to influence $b-0$ and $b_1$ 
]
...
df %>% 
  ...
  geom_segment(aes(x= 6.5, y= m1$coefficients[1]+(6.5*m1$coefficients[2]), 
               ...
               col= "red", lty =2) +
  labs(x="Hours Study", y="Test Score", title = "Leverage Point (red dot)")
...
+ For simple linear model the hat value for a case would be:

$$h_i = \frac{1}{n} + \frac{(x_i - \bar{x})^2}{\sum_i^n(x_i - \bar{x})^2}$$

+ Where;
  + $n$ is the sample size
  + $(x_i - \bar{x})^2$ is the squared deviation of an $x$ value from the mean $x$
  + $\sum_i^n(x_i - \bar{x})^2$ is the sum of all these squared deviations
...
+ Cook's Distance (can be written many ways):

$$D = \frac{standardized Residual}{k+1} * \frac{h_i}{1-h_i}$$

---
#  Cooks Distance 
$$\frac{standardized Residual}{k+1} = Outlyingness $$

$$\frac{h_i}{1-h_i} = leverage$$

+ So $D = outlyingness*leverage$


+ Cook's distance refers to **the average distance the $\bar{y}$ values will move if a given case is removed.**
  + If removing one case changes the predicted values a lot (moves the regression line), then it is influencing our results.
