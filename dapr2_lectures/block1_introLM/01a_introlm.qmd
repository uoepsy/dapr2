...
tibble(
  ...
  mutate(
    Year = Age/12,
    Prediction_cm = 55 + (Age*4),
    Prediction_m = Prediction_cm/100
  ) |>
  kable() |>
  kable_styling(bootstrap_options = "striped", full_width = F)
```
...
+ Our red line is plotted based on the mean length for different ages using [real data](https://www.cdc.gov/growthcharts/who/boys_length_weight.htm)
...
```{r, echo = F, message = F, fig.width = 7, fig.height = 7}
...
basePlot + geom_segment(aes(x = hours, y = predVals, xend = hours, yend = score),
               col = "red", lty = 2) +
  annotate(geom='text', label=test$resids, x=test$hours+.25, y = test$score-test$resids*.5, size=7)
```
...
basePlot + geom_segment(aes(x = hours, y = predVals, xend = hours, yend = score),
               col = "red", lty = 2) +
  annotate(geom='text', label=round(test$resids^2, 2), x=test$hours+.25, y = test$score-test$resids*.5, size=7)
```
...
basePlot + geom_segment(aes(x = hours, y = predVals, xend = hours, yend = score),
               col = "red", lty = 2) +
  annotate(geom='text', label=round(test$resids^2, 2), x=test$hours+.25, y = test$score-test$resids*.5, size=7) +
  annotate(geom='label', label = round(sum(test$resids^2), 2), x = 5, y = 1.5, colour = "red", fontface = 'bold', size = 8)
```
