...
```{r, echo=FALSE, fig.height = 6, fig.width = 5, fig.align="center"}
test |>
...
p <- ggplot() + 
  ...
  labs(x='F', y = '', title = 'F-distribution (2, 147)') +
  theme(
...
+ We can calculate the p-value, that is, the probability of an $F$-statistic at least as extreme as ours (148.9), given $H_0$ is true
...
## When is model comparison useful?

We **shouldn't** use model comparison to...
...
+ In order to apply the $F$-test for model comparison in `R`, we use the `anova()` function (but we are NOT conducting an ANOVA! just using a function that unfortunately has the same name)
...
## Analysis workflow: Block 1

![](figs/workflow-b1.svg)
