...
### Research Question 2
Does the total number of people one follows on Instagram predict FOMO over and above age?
<br />
...
tibble(
Variable = names(dat),
Description = c("Self-reported experience of FOMO; the total sum of 10 questions measured with a 5-point response scale (possible range = 10-50)", "Participant's age in years", "Self-reported measure of Neuroticism; the total sum of 8 questions measured with a 5-point response scale (possible range = 8-40)", "Self-reported measure of Extraversion; the total sum of 8 questions measured with a 5-point response scale (possible range = 8-40)", "Self-reported measure of Openness; the total sum of 10 questions measured with a 5-point response scale (possible range = 10-50)", "Self-reported measure of Agreeableness; the total sum of 8 questions measured with a 5-point response scale (possible range = 8-40)", "Self-reported measure of Conscientiousness; the total sum of 9 questions measured with a 5-point response scale (possible range = 9-45)", "Total number of accounts the participant follows on Instagram")
...
Next, we'll create separate histograms for the `FOMO`, `Age`, and `TotalFollowing` variables. You can produce a single, basic histogram using the following code:
...
```
```{r}
ggplot(dat, aes(TotalFollowing)) + 
  geom_histogram(binwidth = 50, color='black', fill='darkred') +
  labs(x="Total Accounts Following on Instagram", y='Count') +
  theme(axis.title = element_text(size = 14, face='bold'), 
        axis.text = element_text(size = 12))
```
...
<br />
Typically, when we report our results, we need to present descriptive data about our variables. These are typically best presented in table format. Rather than individually calculating descriptive data for each variable, we can use the `describe` function to summarise the descriptives.
...
`describe` produces a wide range of descriptive data. We won't need to include all of it in our table. In this example, we'll just produce a table with the mean and standard deviation of our variables. We could use the `select` function to specify our mean and standard deviation columns, and while this would give us a perfectly good table, do you notice any problems?
...
```{r descriptives}
tableDat <- tibble(Variable=c('Fear of Missing Out', 'Age', 'Neuroticism', 'Extraversion',
                               'Openness', 'Agreeableness', 'Conscientiousness', 
                               'Total Following on Instagram'),
...
(descTable <- tableDat %>%
  ...
  column_spec(1, bold = T))
```
The second example shows a better way to report results; it gives the full names of the variables, rather than their labels in R.
...
```{r}
m2 <- lm(FOMO~Age+TotalFollowing, dat)
anova(m1, m2)
```
...
We might be interested in visualising the association between the significant predictors and the outcome variable. We can do this using scatterplots:
...
In this analysis, we examined the effects of age, total number of Instagram accounts one follows, and personality characteristics on fear of missing out (FOMO). See Table \@ref(tab:descriptives) for descriptive data. $\alpha$ was set to .05 for all analyses.

Our results indicate that age was a significant predictor of fear of missing out ($\beta=`r round(m1Sum$coefficients['Age', 'Estimate'],2)`, CI=[`r round(confint(m1)['Age',], 2)`], SE=`r round(m1Sum$coefficients['Age','Std. Error'], 2)`, p<.001$), $F(`r round(m1Sum$fstatistic['numdf'])`, `r round(m1Sum$fstatistic['dendf'])`)=`r round(m1Sum$fstatistic['value'], 2)`$. Age explained `r round(m1Sum$r.squared*100, 2)`% of the variance in FOMO scores. For every one-year increase in age, participants' FOMO scores decreased by -0.22 points. This suggests that older participants experienced less fear of missing out than younger participants (see Figure \@ref(fig:ageFig))
...
The total number of people one followed on Instagram predicted FOMO over and above age, $F(`r RQ2test$Res.Df[2]`, `r RQ2test$Df[2]`)=`r round(RQ2test$F[2], 2)`$. Together, age & Instagram following explained `r round(m2Sum$adj.r.squared*100, 2)`% of the variance in FOMO scores. See Table \@ref(tab:m2Model) for specific model results.  
<br />
```{r m2Model, echo = F}
tab_model(m2, title = "(\\#tab:m2Model) `Instagram Following Model Results", show.se = T, 
          pred.labels = c('Intercept', 'Age', 'Total Following on Instagram'),
          dv.labels = 'Fear of Missing Out')
```
<br />
For the final analysis, all scores were converted to z-scores. Personality significantly predicted FOMO scores, $F(`r round(m3Sum$fstatistic['numdf'])`, `r round(m3Sum$fstatistic['dendf'])`)=`r round(m1Sum$fstatistic['value'], 2)`, p < .001, R^2_{adj}=`r round(m3Sum$adj.r.squared, 2)`$. For every standard deviation unit increase in neuroticism, FOMO scores increased by `r round(m3Sum$coefficients['Nz','Estimate'], 2)` standard deviation units ($p<.001$). Conscientiousness ($p<.001$), and agreeableness ($p<.001$) were also significant predictors of FOMO scores; Extraversion and Openness were not significant. See Table \@ref(tab:m3Model) for full model results.
