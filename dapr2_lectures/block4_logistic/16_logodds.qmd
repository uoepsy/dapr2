...
**Our goal is to use binary outcome data to estimate the probability of success.**
...
So far, we've been using linear models to model outcomes that's continuous and numeric.

Probabilities are numeric, but **they are not continuous: they are bounded between 0 and 1.**

And standard linear models aren't suitable for bounded outcome variables.

If we try to fit a line to 0/1 data directly, we'll end up estimating impossible probabilities like 120% or –40%.
...
p_ynum <- tibble(
  ...
  error = rnorm(21, 0, 3),
  y_num = 5 + -x + error,
  y_bin = ifelse(y_num >= 0.5, 1, 0)
) |>
  # ggplot(aes(x = x, y = y_bin)) +
  ggplot(aes(x = x, y = y_num)) +
  geom_point(size=3) +
...
The solution: **transform the probabilities into a continuous space, and in that continuous space, we can use a linear model.**
...
(Note: You'll sometimes also hear "odds" called "odds ratios" because the fraction above is the ratio of two probabilities.)
...
p_odds <- tibble(
  ...
  theme(plot.margin = margin(t = 10, l = 10, r = 30))
...
p_odds +
  # horiz
  geom_segment( x = prob, xend = 0, y = odds(prob), yend = odds(prob), colour = dapr2red, arrow = arrow(), linewidth = 2) +
  # vert
  geom_segment( x = prob,  xend = prob,  y = 0, yend = odds(prob), colour = dapr2red, arrow = arrow(), linewidth = 2 ) +
  ggtitle('Prob of 70% = odds of 2.333...')
```

:::{.dapr2callout .fragment}
What about a probability of 20%? or 98%? or 1%?

:::hcenter
:::woo
https://app.wooclap.com/events/AANZIUX/
:::
:::
:::
...
![](figs/16-odds-scale.png){fig-align="center"}
...
## Logarithms to the rescue {visibility="uncounted"}
...
p_log +
  # horiz
  geom_segment( x = odds, xend = 0, y = log(odds), yend = log(odds), colour = dapr2red, arrow = arrow(), linewidth = 2 ) +
  # vert
  geom_segment( x = odds,  xend = odds,  y = -6.22, yend = log(odds), colour = dapr2red, arrow = arrow(), linewidth = 2 ) +
...
![](figs/16-logodds-scale.png){fig-align="center"}

<br>
<br>

:::dapr2callout
**Take-home message:** Probabilities are bounded between 0 and 1, so we can't model them with a regular linear model. 
Instead, we transform them into log-odds, which are continuous (can range between $-\infty$ and $\infty$) and model *that* with a linear model.
More on the modelling next week!
:::
...
**In the two steps we've seen:**

```{r echo=F}
p_odds +
  ggtitle('Probabilities and odds')
```
...
p_logistic +
  # vert
  geom_segment( x = logodds, xend = logodds, yend = plogis(logodds), y = 0, colour = dapr2red, arrow = arrow(), linewidth = 2) +
  # horiz
  geom_segment( x = logodds,  xend = -5,  y =  plogis(logodds),  yend =  plogis(logodds),  colour = dapr2red, arrow = arrow(), linewidth = 2 ) +
  ggtitle(paste0( logodds, ' log-odds = ', 100 * round(plogis(logodds), 3), '% probability'))
```



## From log-odds to probabilities: `plogis()`, <br> the logistic function (aka inverse logit function) {visibility="uncounted"}
...
p_logistic +
  # horiz
  geom_segment( x = -5, xend = qlogis(prob), y = prob, yend = prob, colour = dapr2red, arrow = arrow(), linewidth = 2 ) +
  # vert
  geom_segment( x = qlogis(prob),  xend = qlogis(prob),  y = prob,  yend = 0,  colour = dapr2red, arrow = arrow(), linewidth = 2 ) +
  ggtitle(paste0(prob*100, '% probability = ', round(qlogis(prob), 2), ' log-odds'))
```


## From probabilities to log-odds: `qlogis()`, <br> the logit function {visibility="uncounted"}
...
**This is important because it impacts how we'll interpret logistic regression coefficients next week** (since the slope of a line is the difference between two values!)
...
tibble(
  ...
  # horiz green
  geom_segment(x = 0, y = plogis(0), xend = -5, yend = plogis(0), colour = '#2B8654', linewidth=2) +
  geom_segment(x = 1, y = plogis(1), xend = -5, yend = plogis(1), colour = '#2B8654', linewidth=2) +
  geom_text(
    x = -4.8,
    y = 0.62,
    colour = '#2B8654',
  ...
  # vert green
  geom_segment(x = 0, y = 0, xend = 0, yend = plogis(0), colour = '#2B8654', linewidth=2) +
  geom_segment(x = 1, y = 0, xend = 1, yend = plogis(1), colour = '#2B8654', linewidth=2) +
  geom_text(
    x = 0.5,
    y = 0.15,
    colour = '#2B8654',
  ...
  # horiz pink
  geom_segment(x = 3, y = plogis(3), xend = -5, yend = plogis(3), colour = '#FF6D9A', linewidth=2) +
  geom_segment(x = 4, y = plogis(4), xend = -5, yend = plogis(4), colour = '#FF6D9A', linewidth=2) +
  geom_text(
    x = -4.8,
    y = 0.91,
    colour = '#FF6D9A',
  ...
  # vert pink
  geom_segment(x = 3, y = 0, xend = 3, yend = plogis(3), colour = '#FF6D9A', linewidth=2) +
  geom_segment(x = 4, y = 0, xend = 4, yend = plogis(4), colour = '#FF6D9A', linewidth=2) +
  geom_text(
    x = 3.5,
    y = 0.15,
    colour = '#FF6D9A',
...
Plot the proportion of each level of `agegroup` for each outcome (`taken` = 0 or 1).
...
- Outcome variables that can only have one of two levels (e.g., yes/no, correct/incorrect, ADHD/not ADHD).
- One level must be defined as a "success", represented numerically as 1, and the other level as a "failure", represented numerically as 0 (these are statistical terms and don't need to map to "good" or "bad" levels).
:::

::: {.dapr2callout}
**Why does standard linear regression fail when we try to model binary outcomes?**

- If we fit a standard linear model to binary outcome data, it will estimate probabilities of success that are impossible, that is, outside the range of 0 to 1.
...
- Log-odds are a transformed version of probabilities that is continuous (in other words, log-odds can range between $-\infty$ and $\infty$, unlike probabilities).
:::
...
- The inverse logistic function, or logit function, converts probabilities into log-odds. In R, that's `qlogis()`.
- The logistic function converts log-odds into probabilities. In R, that's `plogis()`.
:::

::: {.dapr2callout}
**Why do *differences* between log-odds not map directly to differences between probabilities?**

- The logistic transformation is not linear, so differences on the log-odds scale can map to various differences on the probability scale, depending on *where* the differences on the log-odds scale originally were.
:::
