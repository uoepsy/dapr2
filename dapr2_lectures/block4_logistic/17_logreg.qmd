...
- What scale are logistic regression coefficients estimated on?
  - log-odds
- How is interpreting logistic regression coefficients different from standard regression coefficients?
  - it's not: everything you learned about predictors equalling zero, holding things constant, etc. still applies for logistic regression
  - the only difference is that the outcome scale is log-odds, not "test score" or "wellbeing" or whatever else we've been using in the first 15 weeks
- How can we use a logistic regression model to predict the probability of "success" (whatever we've defined "success" to mean for our analysis)?
  - same as always, with one extra transformation step at the end!
  ...
  - work out the math to get a single model-predicted value (same as always up til here)
  - new thing: that value is in log-odds space, so use `plogis()` to transform it into a probability
...
<!-- 	- plogis(intercept) -->
<!-- 	- plogis(intercept + slope * predictor) -->
<!-- 	- subtract those -->
