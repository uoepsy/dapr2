...
- **This estimate is significantly different from zero ($p$ < 0.001).**
  ...
  - On the log-odds scale, being different from zero is actually interesting! 
...
## Practice predicting log-odds by hand
...
- What are the log-odds of taking a marshmallow for a one-year-old?
- A three-year-old?
- A ten-year-old?
...
:::hcenter
:::woo
https://app.wooclap.com/events/ODCXCIP/
:::
:::
...
- For children below the age of 6 (the reference level), the estimated log-odds of taking the marshmallow immediately are 0.94. 
  - Equivalently, the estimated probability of taking the marshmallow immediately is `plogis(0.94)` = 71.9%.
...
After substituting the model-estimated coefficients into the linear expression, we get:
...
What value do we substitute in for $\text{agegroup}_{6+}$ to predict the probability of taking the marshmallow for:
