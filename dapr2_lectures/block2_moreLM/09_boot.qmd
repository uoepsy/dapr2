...
**Unclear—I'm not sure why we use this word.**
...
This strategy doesn't rely on the same mathematical assumptions.
...
:::{.r-stack}
![](figs/boot-schematic-1.svg){.fragment fig-align="center" height="600"}

![](figs/boot-schematic-2.svg){.fragment fig-align="center" height="600"}

![](figs/boot-schematic-3.svg){.fragment fig-align="center" height="600"}
:::
...
**Population:** All numbers from 1 to 100 (population mean = 50.5)
...
**Think of bootstrapping as a useful approximation, not a perfect estimate.**

- The main sample was not a perfect representation of the population (different mean, for example).
- Bootstrapping is only as good as the main sample.
...
<!-- **BIG divergences toward the extremes.** -->
...
<!-- I'm not willing to assume that these residuals are normally distributed, nor that they have equal variance. -->
...
<!-- These assumptions might be violated because the model is mis-specified (e.g., key covariates are missing?) -->

<!-- <br> -->

<!-- However, if we're sure that the model is as good as it can be, then we can bootstrap its estimates. -->

<!-- **Bootstrapping is a way of estimating variability that doesn't depend on residuals being normally-distributed and having equal variance.** -->
...
**NOTE: You will never need to do bootstrapping step by step, the way I'm about to show you**.

My goal is to show you what happens behind the scenes inside R.

That way you'll know what's going on when you run the code.

:::

# Bootstrapping LM, step by step
...
Doesn't look good for equal variance of errors...



## Bootstrapping LM, step by step (Sample 1)
...
3. Do 1 and 2 again and again. We'll build a sampling distribution of intercept estimates and a sampling distribution of slope estimates.
:::
...
:::
::: {.column width="35%"}
```{r echo=F, warning = F, message = F, fig.width = 5, fig.height = 4}
boot_plot_list[[1]]  +
  ylim(-3.5, 5.5)
```
:::
::: {.column width="5%" .fragment fragment-index=1}
**(2)**
:::
...
:::: {.columns}
::: {.column width="5%" .fragment fragment-index=2}
**(3)**
:::
::: {.column width="47%" .fragment fragment-index=2}
```{r echo=F, fig.width = 8, fig.asp = .5, fig.align='center'}
boot_coefs_df |>
  head(1) |>
...
## Bootstrapping LM, step by step (Sample 2)
...
## Bootstrapping LM, step by step (Sample 3)
...
$\leftarrow$ As we draw more bootstrapped samples, the mean intercept (solid line) will match the original sample intercept better and better.

<br>

The standard deviation of the sampling distribution (dotted lines) = the standard error of the intercept, which we use to calculate the intercept's 95% CI.

<br>

**We can't trust the original sample's 95% CI (because assumptions weren't met), but we *can* trust the bootstrapped 95% CI.**
:::
::::
...
$\leftarrow$ As we draw more bootstrapped samples, the mean slope (solid line) will come to match the original sample's slope.

<br>

The standard deviation of the sampling distribution (dotted lines) = the standard error of the slope, which we'll use to calculate the slope's 95% CI (which we can actually trust).

<br>

:::dapr2callout
**Take-home message: Bootstrapping is a way of estimating a parameter's standard error, and thus 95% CI, when assumptions about errors aren't met.**
:::
...
Now `summary()` will give us a standard error for each parameter (which is the same as the standard deviation of each parameter's sampling distribution).
...
## Our bootstrapping closely matches `Boot()`
...
```{r echo=F, fig.width = 7, fig.height = 7.5, fig.align='center'}
...
- we can reject the H0 that the Intercept = 0 at $\alpha$ = .05.
- we can reject the H0 that the slope = 0 at $\alpha$ = .05.
...
> Because the assumptions of normality of errors and equal variance of errors were not met, all the standard errors and 95% CIs we report here were estimated from 1000 bootstrapped samples.
...
- These days, people use it to mean that you're independent, a self-starter.
- We think that "bootstrapping" is used in that sense: **you can estimate variability from just a single sample, so in a way, your sample is "pulling itself up by its bootstraps"** (?)
...
- bootstrapping **actually** repeatedly draws samples **from our original sample**
...
If we're using a **95% CI**, then under hypothetical repeated sampling:
...
If we're using a **60% CI**, then under hypothetical repeated sampling:
