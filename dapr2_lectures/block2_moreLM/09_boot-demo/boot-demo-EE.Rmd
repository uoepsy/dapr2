---
title: "DAPR2 09 bootstrapping demo EE"
output: 
  html_notebook:
    toc: true
---
...
```{r}
samp_distrib_sd <- sd( sampling_distrib$sample_mean )
samp_distrib_sd
```
...
## 95% confidence interval based on sampling distribution

```{r}
quantile(sampling_distrib$sample_mean)
```

```{r}
quantile(
  sampling_distrib$sample_mean,
  probs = c(0.025, 0.975)
)
```
