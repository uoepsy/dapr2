...
## Warm-up: Think back to treatment coding

<br>

Challenge yourself from memory (your long-term memory will be stronger if you do!)

**With a treatment-coded categorical predictor, what do a model's intercept and slope mean?**
...
:::hcenter
:::woo
https://app.wooclap.com/events/TFBJACH/
:::
:::
...
| Treatment coding | Estimated mean outcome of reference level | Estimated mean outcome of ref. level = 0 |Non-ref. level mean minus ref. level mean | Non-ref. level mean minus ref. level mean = 0 |
| Sum coding |  |  |  |  |
|  |  |  |  |  |
...
By default, the alphabetically first level is coded as 0.
...
By default, the alphabetically first level is coded as 1.
:::
::::
...
**Predict to yourself/your neighbour:**

- What will a sum-coded line's **intercept** represent?
- What will a sum-coded line's **slope** represent?
:::
...
It's also possible to leave out the `(2)` and just write

```{r}
contrasts(score_data$study) <- contr.sum
contrasts(score_data$study)
```
...
## What does each coefficient represent? {visibility="uncounted"}
...
|---|---|---|---|---|
|  | **Meaning** | **H0** | **Meaning** | **H0** |
|  Treatment coding | Mean outcome of reference level | Mean outcome of ref. level = 0 |Non-ref. level mean minus ref. level mean | Non-ref. level mean minus ref. level mean = 0 |
| Sum coding | Grand mean (mean of all group mean outcomes) |  | Mean of level coded as 1 minus grand mean |  |
|  |  |  |  |  |
...
- Null hypothesis: The mean score of `alone` minus the grand mean is equal to zero.
...
|  | **Meaning** | **H0** | **Meaning** | **H0** |
| Treatment coding | Mean outcome of reference level | Mean outcome of ref. level = 0 |Non-ref. level mean minus ref. level mean | Non-ref. level mean minus ref. level mean = 0 |
| Sum coding | Grand mean (mean of all group mean outcomes) | Grand mean = 0 | Mean of level coded as 1 minus grand mean | Mean of level coded as 1 minus grand mean = 0 |
|  |  |  |  |  |
...
Sum coding has no "reference level".
Instead, the level coded as all –1 is the **"dropped level".**




## No slope is estimated for the dropped level
...
m2_grand_mean <- mean(c(mean_self, mean_summ, mean_read))

p1 <- score_data |> 
  filter(method %in% c('self-test', 'read')) |>
  mutate(method_num = ifelse(method == 'self-test', -1, 1)) |>
  ...
  # grand mean
  geom_hline(yintercept = m2_grand_mean, linewidth = 1.5) +
  labs(
...
p2 <- score_data |> 
  ...
  # slope lines
  geom_segment(aes(x = 1, xend = 1, y = m2_grand_mean, yend = mean_summ), colour = dapr2red, linewidth = 2) +
  # grand mean
  geom_hline(yintercept = m2_grand_mean, linewidth = 1.5) +
  labs(
    x = 'method (in numeric space,\nsecond predictor)'
...
The thick black line shows the grand mean = the model's intercept.

**Once sum coding uses >2 levels, the two-dimensional line between group means does not give us the correct intercept or slope.**
(In a 3D representation, the plane between group means *would* have the right grand mean and slopes.)




## The mathematical model formulation for a <br> sum coded predictor
...
- Slope = Mean of level coded as 1 minus grand mean (i.e., group mean – grand mean)
:::
...
- There'll be a predictor called `methodread`. What is its value?
- There'll also be a predictor called `methodsumm`. What is its value?
...
There's no coefficient for `self-test` because it's the dropped level (coded as all –1s).

Having a dropped level is weird, but it's mathematically necessary for sum coding to work.
...
Let's practice: I'll show you a few different research questions, and you vote on which contrast coding scheme to use.
...
- Sum coding. (Outside of DAPR, you might also see it called "effects coding", "deviation coding", or "sum-to-zero coding".)
- Treatment coding uses 0/1, and one level of the predictor (the one coded as 0) is the reference level.
- Sum coding uses –1/1, and there is no reference level. The level coded as –1 is the "dropped" level.
:::
...
- Slope: The mean of the group coded as 1 minus the grand mean.
:::
...
- The intercept's H0: The grand mean of the outcome is equal to zero.
- The slopes' H0: The mean of the group coded as 1 minus the grand mean is equal to zero (if multiple regression: holding other predictors constant).
:::
