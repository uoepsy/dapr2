...
## Warm-up: Think back to intercepts and slopes

<br>

:::hcenter
:::woo
https://app.wooclap.com/events/MBZKOZE/
:::
:::


## What are intercepts? What are slopes?
...
A line's **intercept** is its y-value (its outcome value) when x (the predictor) is 0.
...
## Categorical predictors with two levels, <br> aka binary predictors {visibility="uncounted"}
...
## Treatment coding represents one level as 0, <br> and the other level as 1
...
## What does it mean to represent levels as 0 or 1? {visibility="uncounted"}
...
We can find parameter values all on our own, without a linear model.

So why do we need to do statistical modelling?

<br>

Because what we can't easily do by hand is statistically test research questions like:
...
```{r plot xy ref alone, echo = F, fig.width = 8, fig.height = 7}
set.seed(1)
p_xy_study +
...
```{r plot xy ref others, echo=F, fig.width = 8, fig.height = 7}
score_data |>
...
<!-- could woo, but probably not enough time -->
...
- The first dummy variable will compare `summarise` back to `read`.
- The second dummy variable will compare `self-test` back to `read`.

:::
::::

## Two dummy variables for three levels

:::: {.columns}
::: {.column width="50%"}

First dummy variable: `summarise` vs. `read`.
...
Second dummy variable: `self-test` vs. `read`.
...
- Each **column** contains one dummy variable.
  1. `summarise` compares `summarise` (the 1 in that column) to `read`.
...
Why is `studyself-test` not significant in the first model but significant in the second model?
...
![](figs/workflow-05.svg){fig-align="center"}
...
- Represent the variable numerically, for example using treatment coding.
...
- For categorical predictors with >2 levels, multiple "dummy variables" individually compare each non-reference level back to the same reference level.
...
- Slope (also written as $\beta_1$, $\beta_2$, etc., or for short, $\beta_j$):
  - the mean outcome for the (current) non-reference level
  - MINUS
  - the mean outcome for the reference level
  - (if multiple regression: while holding other predictors constant)
:::
...
- By default, R uses treatment coding. And by default, the reference level is the level that comes first in the alphabet.
- The intercept's H0: The mean outcome for the reference level is equal to zero.
- The slope's H0: The difference between the mean outcome for the non-reference level and the mean outcome for the reference level is equal to zero.
:::
...
- Another example of a contrast coding scheme is sum coding (which uses –1 and 1). We'll get to know sum coding next week.
:::
