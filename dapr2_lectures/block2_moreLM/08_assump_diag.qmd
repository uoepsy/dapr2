...
**The solution for single/multiple regression models: Component+residual plots** (CR plots, aka "partial-residual plots").
...
**There's a bit of deviation on the left-hand plot, but not enough to force us to stop using a linear model. And the right-hand plot looks pretty good!**
...
:::
::: {.column width="75%"}
Sometimes the model is misspecified and is missing important predictors or interaction terms (mroe on interactions next term!), so adding them in may help account for some of this unequal variance.
:::
::::
