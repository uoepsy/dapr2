...
![](figs/gen-process.svg){fig-align="center" height="250"}
...
**The solution for single/multiple regression models: Component–residual plots.**
...
Basically, component–residual plots can look at the linearity of each association individually, without any of the other predictors getting in the way.
...
:::
::: {.column width="75%"}
Report the non-linearity in your write-up (then your reader knows that your model's estimates are a bit less trustworthy). Totally fine if the deviation isn't huge.
:::
::::
...
:::
::: {.column width="75%"}
Transform outcome or predictor to make errors more normally distributed (e.g., if your distribution of errors is very right-skewed like in this example, taking the logarithm of the outcome is often a good trick).
:::
::::

:::: {.columns}
::: {.column width="25%"}
<img style="margin-top:-10px; width: 2em;" src="figs/lucide-lab--pepper-chilli.svg"> <img style="margin-top:-10px; width: 2em;" src="figs/lucide-lab--pepper-chilli.svg">
:::
::: {.column width="75%"}
Bootstrap your model's estimates. More on this next week!
:::
::::
...
A "residuals vs fitted" plot shows:
...
## Residuals-vs-fitted plots also check linearity


Before, we saw that each predictor's linearity of association can be checked using component-residual plots.
But sometimes those plots aren't possible (e.g., in Block 3 when our models contain interactions).

So it's good to know that **we can also assess linearity of association for the model overall by looking at the wobbly red line in the residuals-vs-fitted plot.**
...
We want the red line to be straight and flat, closely matching the (tiny) dotted black line at y = 0.

**Linearity-wise, there's a bit of deviation on the left-hand plot, but not enough to force us to stop using a linear model. And the right-hand plot looks pretty good!**
...
:::: {.columns}
::: {.column width="25%"}
<img style="margin-top:10px; width: 2em;" src="figs/lucide-lab--pepper-chilli.svg">
:::
::: {.column width="75%"}
Keep the variable as-is and report the non-equal variance in your write-up (then your reader knows that your model's estimates are a bit less trustworthy).
:::
::::
...
:::
::: {.column width="75%"}
Sometimes the model is misspecified and is missing important predictors or interaction terms (more on interactions next term!), so adding them in may help account for some of this unequal variance.
:::
::::
...
A sensitivity analysis asks: **Do our conclusions change if we leave out the unusual data point?**

Specific coefficient values will almost definitely change, but **our overall conclusions are based on whether or not we reject the H0 at our chosen $\alpha$ level, usually .05.**

- If our conclusions don't change, then the influential data point doesn't actually matter.
- If our conclusions DO change, then we need to report this limitation and tell the reader they probably shouldn't believe the original analysis too much.
...
Will the pattern of significance at $\alpha = .05$ change?
...
Technically speaking, **the variance of its estimates increases,** and a significant result is less likely.
...
![](figs/workflow-08.svg){fig-align="center"}
