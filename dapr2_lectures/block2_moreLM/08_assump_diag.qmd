...
**Investigate by plotting the residuals against the fitted (predicted) outcome values.**
...
- A random-looking cloud of data points with the same vertical distance from 0 across the whole x axis
...
:::
:::{.column width="33%"}
**Both outlying and high leverage:**
...
## TODO Compute Cook's distance with `cooks.distance()`
...
## TODO Cook's distance can give us "false alarms"
...
Imagine we want to know whether there's a significant positive association between **x** and **y**, but that one data point has a really big Cook's distance, above 0.15.
...
Only keep rows where `D`, Cook's distance, is smaller than 0.15:

```{r}
df_infl_sens <- df_infl |>
  filter(D < 0.15)
...
And fit the same model as before:

```{r}
m_sens <- lm(y_infl ~ x, data = df_infl_sens)
```
