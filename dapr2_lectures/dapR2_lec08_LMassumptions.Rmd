...
+ If we give `plot()` a linear model object, we can automatically get 6 useful plots.
...
+ Produces a line that follows the data.
...
lin_m1 <- df %>%
  ...
  labs(x= "Hours Study", y="Test Score", 
       title = "Scatterplot with linear (blue) and loess (red) lines")
```
]
...
+ **Assumption**: The residuals ($\epsilon_i$) are normally distributed around each predicted values.
