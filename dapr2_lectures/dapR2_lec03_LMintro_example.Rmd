---
title: "dapR2_lec03_CodingExample"
author: "Tom Booth"
date: "01/08/2020"
...
Let's break down what this is doing. The $\sum_{i}^{n}$ is telling us that we need to perform the action that follows for each individual ($i$) for all rows of data ($n$). 
...
So now to calculate $SSx$. Let's look at the equation again:
...
Well, we have already calculated the individual deviations ($(x_i - \bar{X})$). They are the column called `dev_hours` in the `calc` table. So all we need to do is square these for each observation and sum them. 
...
$$
b_1 = \frac{SP_{xy}}{SS_x} = \frac{21.75}{20.625} = 1.054545
$$
...
```{r}
b0 <- mean(calc$score) - (b1*mean(calc$hours))
b0
```
