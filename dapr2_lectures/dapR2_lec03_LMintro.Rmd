...
+ We can calculate correlations for different data types. For now, we will focus on two numeric (continuous) variables. 
...
---
# Linear model
+ What we will focus on for the entirity of dapR2 is how we move from the idea of an association, to estimating a model for the relationship.
...
---
# Key difference to last week
+ When introducing a model last week, we specified specific values.
...
+ Typically we are looking to estimate the values (e.g. 55 & 4) from some data.

+ Our data here would be information on the `Length` (outcome, DV, Y) and `Age` (predictor, IV, x) of some babies.
...
???
+ In our example, intercept = for someone who doesnt study, what score will they get?
...
+ $\epsilon_i$ = resdiual (we will come to this shortly)
...
--
+ **What does it mean that the intercept ($b_0$) and slope ($b_1$) do not have the subscript $i$?**
...
+ The best fitting line is found using **least squares**
    + Minimizes the distances between the actual values of $y$ and the model predicted values of $y$
    + Specifically minimizes the sum of the squared deviations

+ This is called the **sum of squared error**

$$SSE = \sum_{i}^{n}[y_i - (b_0 + b_1x_{1i})]^2$$
...
+ Predicted value = $\hat{y}_i$ = the y-value on the line at specific values of $x$
    + Or, the value of the outcome our model predicts given someones values for predictors.
...
$$SP_{xy} = \sum_{i}^{n}(x_i - \bar{X})(y_i - \bar{Y})$$


+ $SS_x$ = sums of squared deviations of $x$:


$$SS_x = \sum_{i}^{n}(x_i - \bar{X})^2$$
...
---
# In our data
+ To see the calculations worked through using R, see the worksheet in LEARN and please watch the next video.
