...
2. Interpret interactions between a quantitative and a binary variable.
...
```{r, eval=FALSE}
int <- lm(salary ~ service + dept + service*dept, data = salary_swap_depts)
summary(int) #<<
```
...
salary %>%
  ggplot(., aes(x = service, y = salary, colour = factor(dept))) +
  geom_point() +
  geom_abline(slope = int$coefficients[2], intercept = int$coefficients[1], col = "red") +
  geom_abline(slope = (int$coefficients[2] + int$coefficients[4]), intercept = (int$coefficients[1] + int$coefficients[3]), col = "blue") +
  #geom_smooth(method="lm", se=FALSE)+
  xlim(0,8) +
  ylim(0, 80) +
  labs(x = "Years of Service", y = "Salary (Thousands)") +
...
salary %>%
  ...
  geom_point() +
  geom_abline(slope = int2$coefficients[2], intercept = int2$coefficients[1], col = "red") +
  geom_abline(slope = (int2$coefficients[2] + int2$coefficients[4]), intercept = (int2$coefficients[1] + int2$coefficients[3]), col = "blue") +
  #geom_smooth(method="lm", se=FALSE)+
  xlim(0,8) +
  ylim(0, 80) +
  labs(x = "Years of Service", y = "Salary (Thousands)") +
...
salary %>%
  ...
  geom_point() +
  geom_abline(slope = int3$coefficients[2], intercept = int3$coefficients[1], col = "red") +
  geom_abline(slope = (int3$coefficients[2] + int3$coefficients[4]), intercept = (int3$coefficients[1] + int3$coefficients[3]), col = "blue") +
  #geom_smooth(method="lm", se=FALSE)+
  xlim(-3,3) +
  ylim(0, 80) +
  labs(x = "Years of Service", y = "Salary (Thousands)") +
...
+ We have considered categorical*continuous interactions for now.
  + Different slopes within groups.
  
+ Considered how the choice of a reference/control group changes the model.

+ Looked at how to visualize and calculate simple slopes.
