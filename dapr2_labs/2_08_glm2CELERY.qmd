...
2. For your visualisations, you will need to specify `as_factor()` when plotting the `Quit_Attempt` variable since this is numeric, but we want it to be treated as a factor **only** for plotting purposes
3. Make sure to comment on any observed differences among the sample means of the four intention conditions.
...
Consider whether or not your models are nested. The [flowchart](https://uoepsy.github.io/dapr2/2425/labs/1_b4_reading.html#linear-models-1) may be helpful to revisit, as well as the [model comparisons - logistic regression flashcards](https://uoepsy.github.io/dapr2/2425/labs/1_b4_reading.html#logistic-models-1)
...
The odds of attempting to quit for the group who reported having no intention to quit were significantly less than 1  ($Odds = `r res[1,1]`,\,\, 95\%\, CI\, [`r paste(res[1,2:3],collapse=", ")`]$), indicating a probability of <.5 that they would attempt to quit.
