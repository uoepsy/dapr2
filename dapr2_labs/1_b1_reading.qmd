---
title: "Block 1 Reading & Revision"
link-citations: true
...
For all of the arguments specified below e.g., `bins = `, `alpha = `, `fill = `, `linewidth = `. `linetype = `, `size = ` etc., you can look up the helper function to see the range of arguments they can take using `?` - e.g., `?fill`
...
and fit the model in `R` as either
...
```{r}
hand_mdl2 <- lm(handspan ~ 1 + height, data = handheight)
hand_mdl2
```
...
Unlike $R^2$, Adjusted-$R^2$ does not necessarily increase with the addition of more explanatory variables, by the inclusion of a penalty according to the number of explanatory variables in the model.Since Adjusted-$R^2$ is adjusted for the number of predictors in the model, this should be used when there are 2 or more predictors in the model. 
- Adjusted-$R^2$ should always be less than or equal to $R^2$


:::blue

In **R**, we can see both $R^2$ and Adjusted-$R^2$ in the second bottom row of the `summary(<modelname>)`,:
