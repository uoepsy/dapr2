...
For an overview of basic statistical tests and core concepts (e.g., $p$-values), please revisit the [DAPR1 materials](https://uoepsy.github.io/dapr1/2324/) for a refresher.
...
`r optbegin("Simple & Multiple Regression Models - Extracing Information", olabel=FALSE,toggle=params$TOGGLE)`
...
ggplot(recalldata, aes(x = age, y = recall_accuracy)) + 
    geom_point(size = 3, alpha = 0.5) +
    geom_smooth(method = lm, se = FALSE) + 
    labs(x = "Recall Accuracy (%)", y = "Age (in years")
```
...
There are three key components to allow you to successfully cross-reference within yoru RMarkdown document:
