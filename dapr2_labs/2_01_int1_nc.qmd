...
+ Interpretation: The simple effect of location (or the difference in wellbeing scores between rural and non rural residents) when number of social interactions is 0.  
    - For residents who had zero social interactions per week, living in a rural location was associated with wellbeing scores `r round(coef(rural_mod)[3],2)` points higher than living in non-rural locations (note that this difference is is not significantly different from zero).  
