...
3. To include facets, within your `ggplot()` argument you will need to specify `+ facet_wrap()` in order to produce facets for each location. It would also be useful to specify `geom_smooth(method="lm")`.
4. To include different colours for each location, you will need to specify `colour = isRural` within your `ggplot()` argument, as well as `+ scale_colour_discrete()` to state the `name =` of your legend and the `labels =` associated with each coloured line.
...
+ Interpretation: The simple effect of location (or the difference in wellbeing scores between rural and non rural residents) when number of social interactions is 0.  
    - For residents who had zero social interactions per week, living in a rural location was associated with wellbeing scores `r round(coef(rural_mod)[3],2)` points higher than those living in non-rural locations (note that this difference is is not significantly different from zero).  
