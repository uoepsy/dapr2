...
```{r}
#We can make all of the changes noted above in one (long command). First we can use the function `factor()` by specifying the current levels and what labels each level should map to. We can also simply rename the Y column to score. 
...
```{r}
ggplot(data = cog, aes(x = Task, y = Score)) + 
  geom_boxplot() + 
...
- a linear model to investigate whether the X
...
Before stating the model, you we first need to define the dummy variables for Diagnosis:
...
**Note**: Fortunately, `R` computes the dummy variables for us! Thus. each row in the `summary()` output of the model will correspond to one of the estimated $\beta$'s in the equation above.
...
```{r}
#plt_dass_mdl <- probe_interaction(model = cog_mdl, 
#                  pred = Diagnosis, 
#                  modx = Task, 
#                  cond.int = T,
#                  interval = T, 
