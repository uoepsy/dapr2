...
+ $\beta_1$ = `DiagnosisAmnesic` = `r round(coef(cog_mdl)[2],2)`
    - The difference in score between Amnesic and Control conditions on the Grammar task
    - Individuals with Amnesia scored 20 points lower than Control participants when completing the Grammar task.
...
+ $\beta_2$ = `DiagnosisHuntingtons` = `r round(coef(cog_mdl)[3],2)`
    ...
    - Individuals with Huntingtons scored 40 points lower than Control participants when completing the Grammar task.
...
+ $\beta_3$ `TaskRecognition` = `r round(coef(cog_mdl)[4],2)`
    ...
    - Individuals in the Control diagnosis condition completing Recognition tasks score 15 points higher than those Control participants completing Grammar tasks.
    
+ $\beta_4$ `DiagnosisAmnesic:TaskRecognition` = `r round(coef(cog_mdl)[5],2)`
    - The difference between score in Control and Amnesic diagnosis conditions between Grammar and Recognition tasks
    - ADD INTERPRETATION

+ $\beta_5$ `DiagnosisHuntingtons:TaskRecognition` = `r round(coef(cog_mdl)[6],2)`
    - The difference between score in Control and Huntington diagnosis conditions between Grammar and Recognition tasks
    - ADD INTERPRETATION
...
The difference in scores between the recognition and grammar tasks, respectively measuring explicit and implicit memory, for Huntingtons patients in comparison to controls was significant and indicated a difference of 40 points in explicit vs implicit memory performance ($t(24) = 3.41, p = .002$).

The difference in scores between the recognition and grammar tasks, respectively measuring explicit and implicit memory, for amnesiac patients in comparison to controls was estimated to be -10 points but it was not found to be significantly different from 0 ($t(24) = -0.85, p = .40$).

In relation to the researchers' hypotheses that the difference in performance between explicit and implicit memory tasks did not differ significantly between amnesic and control patients, whilst it did differ significantly between Huntington and control patients.

Compared to controls, amnesiac patients had a significant deficit in explicit memory (as measured by the recognition task), but not on implicit memory (as measured by the grammar task).

Compared to controls, Huntingtons patients had a significant deficit in implicit memory (as measured by the grammar task) but not in explicit memory (as measured by the recognition task).
