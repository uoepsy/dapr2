...
2.  Have completed previous lab exercises from [Semester 2 Week 7](https://uoepsy.github.io/dapr2/2526/labs/2_07_glm1.html) and [Semester 2 Week 8](https://uoepsy.github.io/dapr2/2526/labs/2_08_glm2.html) 
...
descript <- marshmallow |> 
   ...
   summarise(
       M_Age = mean(age),
       SD_Age = sd(age),
       percent_taken = sum(taken=="1")/n()*100) |>
...
mallow_plt1 <- ggplot(data = marshmallow, aes(x = age, fill = as_factor(taken))) + 
    geom_density(alpha = 0.5) + 
    scale_fill_manual(values = c("0" = "darkgreen", "1" = "red"), labels = c("0" = "waited", "1" = "taken")) + 
    facet_grid(visibility ~ timeofday) +
    labs(x = "Age", fill = "Marshmallow Status")
mallow_plt1

#facet plot - plot probability
mallow_plt2 <- marshmallow |> 
    ggplot(aes(x = age, y = taken, colour = visibility)) +
    ylab("p(taken)") +
    geom_jitter() +
...
To address the research question of whether the probability of taking the marshmallow changed as a function of time of day, age, and marshmallow visibility, we formalised our hypotheses as: 
...
It appeared that when the marshmallow was visible, a greater percentage of children took it; the percentage of children who engaged in delayed gratification was higher in the morning sessions; and that older children were less likely to take the marshmallow (see @fig-taken-plot).
...
The probability of taking the marshmallow when the marshmallow was visible in the morning session for those aged zero was 0.99. There was no association between time of day and taking the marshmallow `r res[2,1]` ($95\%\, CI\, [`r paste(res[2,2:3],collapse=", ")`])$ after accounting for age and visibility. Age and visibility were significant predictors of delayed gratification (see @tbl-res). For every year increase in age, the odds of taking the marshmallow decreased by a factor of `r res[3,1]` ($95\%\, CI\, [`r paste(res[3,2:3],collapse=", ")`])$ when the marshmallow was visible in the morning session. When the marshmallow was hidden (as opposed to visible), the odds of taking the marshmallow decreased by a factor of `r res[4,1]` ($95\%\, CI\, [`r paste(res[4,2:3],collapse=", ")`])$ for those aged zero in the morning session. Thus, we rejected the null hypothesis. 
...
#### Optional/Extra: Mean-Centre Age

`r qbegin(4)`

You might be thinking after writing up your results - why did we not mean-centre age? We don't have a children in the study aged zero, in fact the study only included children aged 3-10 years - so can't we make our interpretation more meaningful? And the answer to that question would be yes! 

Mean center the continuous IV (i.e., age), and re-run your model using this. Interpret your new results int the context of the research question.

`r qend()`

`r solbegin(": Example Write-Up of Optional/Extra Analysis", show = params$SHOW_SOLS, toggle = params$TOGGLE)`

```{r}
#Create mean centered variable for age - store as mc_age
marshmallow <-
    marshmallow |>
    mutate(
        mc_age = age - mean(age)
...
#Re-run model with mc_age
mm2 <- glm(taken ~ timeofday + mc_age + visibility, data = marshmallow, family = binomial)
summary(mm2)

tab_model(mm2,
          dv.labels = "Marshmallow Taken",
          pred.labels = c("Intercept", "Time of Day - PM", "Age (in years; mean centered)", "Visibility - Hidden"),
          title = "Regression Table for Marshmallow Model 2")
```

```{r}
#| include: false
res2 <- exp(car::Confint(mm2))
res2 <- round(res2,2)
```

The probability of taking the marshmallow when the marshmallow was visible in the morning session for those of average age (i.e., 6.5 years old) was 0.64. There was no association between time of day and taking the marshmallow `r res2[2,1]` ($95\%\, CI\, [`r paste(res2[2,2:3],collapse=", ")`])$ after accounting for age and visibility. Age and visibility were significant predictors of delayed gratification (see @tbl-res2). For every year increase in age, the odds of taking the marshmallow decreased by a factor of `r res2[3,1]` ($95\%\, CI\, [`r paste(res2[3,2:3],collapse=", ")`])$ when the marshmallow was visible in the morning session. When the marshmallow was hidden (as opposed to visible), the odds of taking the marshmallow decreased by a factor of `r res2[4,1]` ($95\%\, CI\, [`r paste(res2[4,2:3],collapse=", ")`])$ for those of average age in the morning session. Thus, we rejected the null hypothesis. 

```{r tableres2, echo = FALSE, message = FALSE}
#| label: tbl-res2
#| tbl-cap: "Regression Table for Marshmallow Model 2"
tab_model(mm2,
          dv.labels = "Marshmallow Taken",
          pred.labels = c("Intercept", "Time of Day - PM", "Age (in years; mean centered)", "Visibility - Hidden"),
          title = "Regression Table for Marshmallow Model 2")
...
# Block 4 (Weeks 6-9) Recap

Given that we have now covered all DAPR2 course content, we would also strongly encourage you to start creating your revision materials in advance of the exam. You can access all the flashcards that you've been presented with in this block [here](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html). These will provide a good starting point for collating your notes together on the contents of blocks 1, 2, 3, and 4. We also suggest that you review your weekly quiz feedback (as many of you have learned in Psychology 2A, it is important to provide feedback to allow learners to improve their learning and retention of information, as well as correct any misunderstandings!).
