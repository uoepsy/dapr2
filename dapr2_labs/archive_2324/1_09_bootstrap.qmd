...
2. Have completed previous lab exercises from [Week 2](https://uoepsy.github.io/dapr2/2324/labs/1_02_mlr.html) and [Week 9](https://uoepsy.github.io/dapr2/2324/labs/1_08_assump_diag.html)
...
- You may wish to consider using helpful bin sizes, see the `binwidth` argument, but this is subjective and optional
...
Using `geom_point()` here might not be the best idea, all we would see from the plot is all combinations of Attitude towards Science and Faith and a) Age and b) Science Knowledge that were observed in the data, and this is not very informative:
...
See [DAPR1 Semester 2 Week 10 Lectures](https://uoepsy.github.io/dapr1/2223/lectures/dapR1_lec20_Correlation.html#26) for a recap on how to interpret $r$ values. 
...
The most efficient way to do this is likely to use the `plot(model)` function.

To have all 4 plots display in a nice 2 by 2 matrix, we can use the command `par(mfrow = c(2,2))`. Then create the plot. Make sure to always re-set this back to a single figure made up by a single panel with the command `par(mfrow = c(1,1))`. 
...
You can't use `tab_model()` here, but instead will need to use `kable()` and `kable_styling()`.
