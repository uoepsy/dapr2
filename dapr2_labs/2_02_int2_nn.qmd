...
+ The interaction between SCS score and Neuroticism on DASS-21 Scores - the change in the slope of Neuroticism as a function of SCS Scores.   
    - For every 1 standard deviation increase in SCS scores, when Neuroticism scores increase by 1 SD, the slope with DASS-21 scores was adjusted -1.83.  
...
+ Because we are looking at a numeric x numeric interaction, we want to specify `jnplot = T` (see [this weeks lecture](XXXXXXXXXX), slides 17-21 for a worked example).
...
Full regression results including 95% confidence intervals (\95% CIs) are shown in @tbl-dass-modresults. The $F$-test for model utility was significant $(F(3,652) = 48.50, p<.001)$, and the model explained approximately 17.87% of the variability in DASS-21 scores.
