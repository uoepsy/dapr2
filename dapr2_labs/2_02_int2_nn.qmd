...
2. Have completed previous lab exercises from [Week 1](https://uoepsy.github.io/dapr2/2526/labs/2_01_int1_nc.html)  
...
For examples, see flashcards on [descriptives statistics tables - numeric values only examples](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#numeric-exploration) and [numeric x numeric example - visualise data](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#numeric-x-numeric-example).  
...
Remember that you can specify `geom_smooth()` to add a trend line. For a recap, review the facet examples contained within the [visual exploration flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#visual-exploration). Alternatively, review the [numeric x numeric example - visualise data flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#numeric-x-numeric-example). 
...
scs_study |>
  mutate(
    zn_group = cut(zn, 6)
  ) |> ggplot(aes(x = scs, y = dass, col = zn_group)) + 
  geom_point(alpha=.1) + 
...
Review the [standardisation flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#data-transformations) for a recap if needed. Note, it would be best to create a new z-scored variable to then use within the model in this instance. 
:::
...
For an example, review the [interaction models > numeric x numeric example > model building flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#numeric-x-numeric-example).
...
For an overview of how to interpret coefficients, review the [interaction models > interpreting coefficients flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#interaction-models). It is important to remember that you have standardised the variables in your model, and this will affect your interpretation. For a recap on what standardisation is and how it affects the scales of your variables, review the [standardisation flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#data-transformations). 
   
For a specific example of coefficient interpretation, review the [interaction models > numeric x numeric example > results interpretation flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#numeric-x-numeric-example). 
...
+ The intercept, or predicted DASS-21 score for an individual with an average SCS score and average Neuroticism score. In this case, the mean scores of SCS and Neuroticism are both 0 (since both have been standardised).  
    - An individual with a Neuroticism score of 0 ($Z$-scored) and an SCS score of 0 ($Z$-scored) was expected to have a DASS-21 score of `r round(coef(dass_mdl)[1],2)` points.  

## `zn`  
$\beta_1$ = `zn` = `r round(coef(dass_mdl)[2],2)`  

+ The simple slope of Neuroticism on DASS-21 scores with average SCS scores (0; $Z$-scored).  
    - Specifically for an individual with an average SCS score (0; $Z$-scored), every 1 standard deviation increase in Neuroticism score was associated with a `r round(abs(coef(dass_mdl)[2]),2)` point increase in DASS-21 scores.
...
+ The simple slope of SCS scores on DASS-21 scores with average Neuroticism scores (0; $Z$-scored).    
    - Specifically for an individual with an average Neuroticism score (0; $Z$-scored), every 1 standard deviation increase in SCS score was associated with a `r round(abs(coef(dass_mdl)[3]),2)` decrease in DASS-21 scores.  
...
+ The interaction between SCS score and Neuroticism on DASS-21 Scores - the change in the slope of SCS Scores as a function of Neuroticism.   
    - For every 1 standard deviation increase in SCS scores, when Neuroticism scores increased by 1 standard deviation, the slope with DASS-21 scores was adjusted by `r round(coef(dass_mdl)[4],2)` points. Equivalently, the higher Neuroticism scores, the stronger the negative association between SCS Scores and DASS-21 scores.   
...
For an overview and example, review the [interaction models > numeric x numeric example > model visualisation flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#numeric-x-numeric-example). 
...
For an overview and example, review the [interaction models > numeric x numeric example > model visualisation flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#numeric-x-numeric-example). Pay particular attention to how you can extract specific parts of output. 
...
The Johnson-Neyman technique (see @fig-sim-slopes) indicated that the association between DASS-21 scores and SCS was not significant when Neuroticism scores between -0.55 and -1.28 standard deviations below the mean. 
:::
...
For an example of coefficient interpretation, review the [interaction models > numeric x numeric example > results interpretation flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b3_reading.html#numeric-x-numeric-example). 
