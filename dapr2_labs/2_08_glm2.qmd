...
2. Have completed previous lab exercises from [Semester 2 Week 7](https://uoepsy.github.io/dapr2/2526/labs/2_07_glm1.html)
...
Review the many ways to numerically and visually explore your data by reading over the [data exploration flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#data-exploration).
...
2. For your visualisations, you will need to specify `as_factor()` when plotting the quit attempt variable since this is numeric, but we want it to be treated as a factor **only** for plotting purposes. You can either visualise as counts or as proportions (choose one approach)./    
...
We can visually explore the association between the two categorical variables either as counts or proportions:

```{r}
#| label: fig-smoke-desc1
...
smoke_plt1 <- ggplot(data = smoke, aes(x=as_factor(Quit_Attempt), fill=Intention)) +
  ...
  labs(fill = 'Intention to Quit', x = "Attempted Quitting (0 = No, 1 = Yes)", y = "Count") + 
    theme(legend.position = "none")
smoke_plt1
```

```{r}
#| label: fig-smoke-desc2
#| fig-cap: "Association between Intention and Quitting Attempt"
smoke_plt2 <- ggplot(data = smoke, aes(x=as_factor(Quit_Attempt), fill=Intention)) +
  geom_bar(position = "fill") +
  labs(fill = 'Intention to Quit', x = "Attempted Quitting (0 = No, 1 = Yes)", y = "Proportion")
smoke_plt2
```

::: {.callout-important icon=false appearance="minimal"}

From @fig-smoke-desc1 and @fig-smoke-desc2, we can see that for those who made an attempt to quit using tobacco products, the majority intended to quit - either in the immediate or distant future. For those who made no attempt to quit using tobacco products, the majority either had no intention of quitting or did not plan to quit in the near future. 
...
For the former, you will need to specify `type = "response"` to obtain the predicted probabilities, and this requires the model coefficients to be in log-odds. When specifying `newdata = `, it might be useful to ask for each unique level of our "Intention" variable. This is very similar in procedure to how you've done this in the past. For a recap, see the [model predicted values and residuals flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#model-predicted-values-residuals). For the latter, we can calculate the probability corresponding to a given value of `logit(p)`.    
...
For an overview of logistic model assumptions, review the [model assumptions > logistic models flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#logistic-models-3).   
...
Review the [model comparisons > logistic models > AIC & BIC flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#logistic-models-2)
...
The odds of attempting to quit for the group who reported having no intention to quit were significantly less than 1  ($Odds = `r res[1,1]`,\,\, 95\%\, CI\, [`r paste(res[1,2:3],collapse=", ")`]$), corresponding to a probability of 0.1250. 
...
In comparison to those with no intention to quit, for those who intended to quit in the next six months, the odds of quitting were multiplied by `r res[3,1]` ($95\%\, CI\, [`r paste(res[3,2:3],collapse=", ")`]$).
