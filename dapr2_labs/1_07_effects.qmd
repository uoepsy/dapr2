...
2. Have completed [Week 7](https://uoepsy.github.io/dapr2/2324/labs/1_06_dummy.html) lab exercises
...
3. It might be helpful to add a line representing the grand mean (the mean of all the observations). You can do this by specifying `geom_hline()`. Within this argument, you will need to specify where the horizontal line should cut the y-axis via `yintercept = `. You might want to specify line: type (via `lty = `), width (via `lwd = `), and colour (via `colour = `).
...
+ Predicted mean of "Pop" = $\hat \mu_\text{Pop}$ = 22.1414 - 0.2424 = 21.90
+ Predicted mean of "Classical" = $\hat \mu_\text{Classical}$ = 22.1414 + 2.0328 = 24.17
