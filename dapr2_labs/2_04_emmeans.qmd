...
In terms of reference groups, we should re-level the `Diagnosis` factor so that the reference group is "Control", and the reference group for `Task` is "Recognition". 
...
Use the **stargazer** package to create table:
...
Run contrast analysis, and interpret output. 
...
Considering the simple effects that we just saw above, identify the significant effects and match them to the parts of your interaction plot.

:::{.callout-tip appearance="simple" collapse="true"}

### Hint

Recall that the **patchwork** package allows us to arrange multiple plots using either `/` or `|`
...
First, use the Bonferroni method to adjust for multiple comparisons, and then obtain confidence intervals.

Next, remove the adjustment, and comment on how these $p$-values differ from those obtained via the Bonferroni correction. 
...
There are 9 tests being performed, but there are 3 in each 'family' (each `Task`). In the lecture, we talked about adjusting the $\alpha$ level (i.e., instead of determining significance at $p < .05$, we might adjust and determine a result to be statistically significant if $p < .005$, depending on how many tests are in our family of tests).  
...
In terms of differences in $p$-values, they are thus 1/3 of the size.
