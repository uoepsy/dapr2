...
Out of interest, let's apply both the Tukey and Bonferroni adjustment - the former since we are interested in all pairwise comparisons, and the latter to explore how our adjusted $p$-value differs from our non-adjusted (note, in practice you would only apply one correction and justify this choice based on your design - we are only applying two to note how they differ!):
...
The first Bonferroni adjusted $p$-value is 0.0207. The Bonferroni method simply multiplies the 'raw' $p$-value by the number of the tests. 
```{r}
#for comparison, raw p-value (i.e., unadjusted) was 0.0069

#does Bonferroni give us the same? Yes. 
0.0207/3
```

In terms of differences in Bonferroni to raw $p$-values, they are thus 1/3 of the size.
