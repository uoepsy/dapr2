...
1. Interpret a 2x4 interaction model;
1. Write up its results.
...
If you're having knitting problems, please revisit the [DAPR1 knitting checklist](https://uoepsy.github.io/dapr1/2526/labs/zf_formatting_resources.html#successful-knitting-checklist).
...
# Study -- bar plot of counts
notes |>
  ggplot(aes(x = study)) +
  geom_bar()

# Medium -- bar plot of counts
notes |>
  ggplot(aes(x = medium)) +
  geom_bar()
...
Draft an analysis strategy section based on the above research question and analysis provided.
...
The `notes` dataset contained information on 160 participants who took part in a study concerning the role(s) of note taking and study time on test scores.
Participants took notes on a lecture via one of two mediums: either on a laptop $(n = 80)$ or long-hand using pen and paper $(n = 80)$.
They were then randomly allocated to one of four study time conditions, either engaging in no $(n = 40)$, minimal $(n = 40)$, moderate $(n = 40)$, or extensive $(n = 40)$ study of the notes taken on their assigned medium.
Participants then answered a series of questions based on the lecture content.
The maximum score was 100, where higher scores reflected better test performance. 

Our aim in this report was to address the following research question: Do differences in test scores between study conditions differ by the note-taking medium used? If so, how do they differ?

We checked the data and found that all participant data was complete and that test scores were within a reasonable range (i.e., 0–100).
We coded the categorical variables using dummy coding, with "No" as the reference level for study condition and "Longhand" as the reference level for medium.

To investigate whether study condition (No, Minimal, Moderate, Extensive) and note-taking medium (Longhand, Laptop) interacted to influence test scores, we fit the following model, where $S$ stands for "study" and $M$, for "medium":
...
We tested whether there was a significant interaction between study condition and note-taking medium:
...
We considered effects statistically significant at $\alpha = .05$. 
As we were using between-subjects datasets, we assumed independence of our error terms. 
We also assumed linearity, as all predictor variables were categorical. 
We assessed the assumption about normality of errors using a Q-Q plot of residuals (points should follow along the diagonal line) and the assumption about equal variance of errors using a residual-vs-fitted-values plot (residuals should be evenly spread across the range of fitted values, where the spread should be constant across the range of fitted values).
We will discuss the results of all these analyses in the next section.
...
Draft a results section based on your detailed analysis strategy and the analysis provided.
...
#| label: tbl-descript
#| tbl-cap: "Descriptive Statistics for all combinations of study and medium conditions"
...
In the No and Minimal study conditions, test scores between participants using a laptop or writing longhand are similar, though longhand note-takers show a slight test score advantage over laptop users.
However, in the Moderate and Extensive study conditions, participants in the longhand note-taking condition scored dramatically higher than those using laptops.
Because of this apparent difference of differences, there may be an interaction between study condition and note-taking medium (see @fig-notes-plot). 

```{r fig-notes-plot, echo = FALSE, fig.cap = "", fig.align = "center"}
#| label: fig-notes-plot
#| fig-cap: "Association between test score and medium / study conditions. As the amount of study increases, the test score seems to increase more rapidly for longhand note-takers than for laptop note-takers."
...
The model met assumptions of linearity of association and independence of errors (see Appendix A, top left panel of @fig-assumpt; residuals were randomly scattered with a mean of zero and there was no clear dependence), equal variance of errors (see Appendix A, bottom left panel of @fig-assumpt; there was a constant spread of residuals), and normality of errors (see Appendix A, top right panel of @fig-assumpt; the QQplot showed very little deviation from the diagonal line). 
The predictors were not correlated with one another, with GVIF scores of 1.

We found a significant interaction between study condition and note-taking medium $F(7, 152) = 1081, p < . 001$. Full regression results, including 95\% Confidence Intervals, are shown in @tbl-rq1-results. 

```{r echo = FALSE}
#| label: tbl-rq1-results
#| tbl-cap: Regression coefficients table
tab_model(notes_mdl, 
...
As displayed in @fig-intplot, the difference in scores did differ significantly across the note-taking medium and study conditions, where scores differences appeared to get larger as the period of study increased.
Within each of the four levels of study, the difference between laptop users' scores and longhand writers' scores was significantly different from zero, with longhand writers consistently scoring higher.
Thus, even with no study, longhand note-takers achieved significantly higher scores than laptop users, and this difference grew larger as the amount of study increased.
...
#| label: fig-intplot
#| fig-cap: "Interaction plot, illustrating the model's estimates (mean and 95% CI) for all combinations of study and medium conditions."
plt_notes_mdl
```
...
Draft a discussion section based on your results and the analysis provided.
...
Overall, we were able to reject the null hypothesis that there is no influence from note-taking medium on the effect of different amounts of study on test scores.  
  
Specifically, we found that people who took longhand notes performed better than people who took notes on a laptop, and the advantage of taking longhand notes increased as those note-takers spent more time studying.  
  
Our results suggest that using a laptop to take notes may set learners up for less success, and that engaging in more hours of study is not enough to mitigate this effect.  
...
+ Accurately referred to the labels of the figures and tables presented in the appendix in the main report (if you don't refer to them, the reader won't know what they are relevant to!).
