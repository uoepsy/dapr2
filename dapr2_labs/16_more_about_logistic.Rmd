...
We are super lucky, as we already saved both Pearson residuals and the Deviance residuals (__not the standardized__ and __not the studentized__ ones), so we just need to do respectively:
...
We already computed the required residuals and stored them as `Pres`:
```{r}
...
# UMBERTO! check. add a new ppt in group I = 4, to make sure there is something easier to estimate here. 
qa[62,]<-cbind(S=62,Q=0,I=4)
...
:::int
The odds that participant who reported that they "never intend to quit" did subsequently quit smoking was `r round(exp(coef(qa_fit1))[1],2)` (95% CI [`r paste(round(exp(confint(qa_fit1)),2)[1,],collapse="--")`]). Relative to this group, reporting an intention to quit but not in the next 6 months increased the odds of quitting by `r round(exp(coef(qa_fit1))[2],2)` (95% CI [`r paste(round(exp(confint(qa_fit1)),2)[2,],collapse="--")`]), reporting an intention to quit in the next 6 months increased the odds of quitting by `r round(exp(coef(qa_fit1))[3],2)` (95% CI [`r paste(round(exp(confint(qa_fit1)),2)[3,],collapse="--")`]), and intending to quit in the next 30 days was associated with an increased odds of `r round(exp(coef(qa_fit1))[4],2)` (95% CI [`r paste(round(exp(confint(qa_fit1)),2)[4,],collapse="--")`])
