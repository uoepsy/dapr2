...
ggplot(data = wrdata, aes(x = isRural, y = wellbeing)) +
  geom_boxplot() + 
  labs(x = "Location", y = "Wellbeing Score (WEMWBS)")
```
...
ggplot(data = wrdata, aes(x = social_int, y = wellbeing)) +
  ...
  facet_wrap(~isRural) + 
  labs(x = "Social Interactions (number per week)", y = "Wellbeing Score (WEMWBS)")
```
...
Those in non-rural locations appear to have higher wellbeing scores across all levels of social interactions. There appears to be a greater difference in wellbeing scores by location at the highest number of social interactions.
...
The lines in thetwo plots above are not running in parallel - this suggests the presence of an interaction. Specifically in our example, the non-parallel lines suggested an interaction effect based on location, as the number of social interactions does not appear to have the same influence on rural and non-rural residents' wellbeing scores. 
...
nd = expand_grid(social_int=-0.5:13,isRural=c("rural","not rural"))
nd = nd %>% mutate(wellbeing = predict(rural_mod, newdata = .))
...
p1 <- ggplot(data = wrdata, aes(x = social_int, y = wellbeing, colour = isRural)) +
    ...
    scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 1)) + 
    labs(x = "Social Interactions \n(n per week)", y = "Wellbeing Score", caption = "Note that the lines without SE intervals on the left plot represent \npredicted values below the minimum observed number of social \ninteractions, to ensure that zero on the x-axis is visible") +
...
p2 <- ggplot(data = wrdata, aes(x = mc_social_int, y = wellbeing, colour = isRural)) +
    ...
    geom_vline(xintercept = 0, colour="black", linetype = "longdash", size = 0.5) + 
    scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 1)) + 
    labs(x = "Mean Centred Social Interactions \n(n per week)", y = "Wellbeing Score") +
    scale_colour_discrete(
    name ="Location",
    labels=c("Not Rural", "Rural"))
