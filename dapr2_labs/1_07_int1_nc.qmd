...
p1 <- ggplot(data = wrdata, aes(x = social_int, y = wellbeing, colour = isRural)) +
  ...
  geom_smooth(method="lm", se=TRUE) +
    geom_vline(xintercept = 3, colour="black", linetype = "longdash", size = 1) + 
    scale_colour_discrete(
    name ="Location",
    labels=c("Not Rural", "Rural"))
...
Whilst the difference in rural vs non-rural may not be significantly different when social interactions is zero, there is a significant difference at the average number of social interactions (as you can see from the plot in the hint or below).
...
The association between wellbeing (WEMWBS Scores) and social interactions was found to be dependent upon location (rural/non-rural), with a less positive association between the two for those located in rural locations ($\beta$ = -0.52, $SE$ = 0.16, $p$ = .002). This interaction is visually presented in @fig-int. Therefore, we have evidence to reject the null hypothesis (that the association between wellbeing and social interactions is not moderated by whether or not a person lives in a rural area).
