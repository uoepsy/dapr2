...
or 
$$
\widehat{Wellbeing} = \hat \beta_0 + \hat \beta_1 \cdot Social Interactions + \hat \beta_2 \cdot Location_{Rural}  + \hat \beta_3 \cdot (Social Interactions \cdot Location_{Rural})  
$$
...
+ $\beta_0$ = `(Intercept)` = `r round(coef(rural_mod)[1],2)`
    ...
    - Interpretation: The intercept, or predicted wellbeing score when the number of social interactions per week is 0, and when location is rural.
...
+ $\beta_1$ = `social_int` = `r round(coef(rural_mod)[2],2)`
    ...
    - Interpretation: The simple slope of social interactions (number per week) for location reference group (rural).
...
+ $\beta_3$ `social_int:isRuralrural` = `r round(coef(rural_mod)[4],2)`
    ...
    - Interpretation: The interaction between social interactions (number per week) and location (rural/non-rural) - the difference in the simple slopes of social interactions for rural vs non-rural residents. 
...
Provide key model results in a formatted table, and make reference to your interaction plot.
...
tab_model(rural_mod,
          ...
          pred.labels = c("social_int" = "Social Interactions (number per week)",
                          ...
                          "social_int:isRuralrural" = "Social Interactions * Location - Rural"),
...
Full regression results including 95% Confidence Intervals are shown in @tbl-rural-modresults. The $F$-test for model utility was significant $(F(3, 196) = 27.49, p<.001)$, and the model explained approximately 28.54% of the variability in wellbeing scores. See @fig-int

The association between wellbeing scores and social interactions was dependent upon location (rural vs not rural), with a greater negative association between the two for those in rural locaions (β = -0.52, SE = 0.24, p = .002). This interaction is visually presented in Figure 3.
