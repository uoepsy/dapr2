...
In the t-test, `mean in group Colour` = 38.10 corresponds to the estimate intercept of the linear model, $\hat \beta_0 = 38.10$. That is, $$\bar y_C = \hat \beta_0$$
...
:::yellow
We can't confirm if an assumption holds. We can only check related conditions that can highlight violations of that assumption.

However, be wary to jump from *"no sign of violations"* to *"the assumption holds"* as we might have no sign of violations while the assumption does not hold.

`r optbegin('Give me an example', FALSE, show = TRUE, toggle = params$TOGGLE)`
**~~Schrodinger's~~ Josiah's cat**

There is a cat in a box. 
We cannot tell if the cat is dead, we can only check the related conditions that could indicate that it is alive (is the box moving? is the box meowing?). 
We might have no signs of life, but the cat may be perfectly happy and alive..
`r optend()`
:::
...
Here are some calculations that will be useful:
```{r}
...
:::int
The residuals appear to be randomly scattered around zero in both groups, without pattern with respect to the fitted values. Hence, there is no sign of violation of the zero-mean assumption.
:::
`r solend()`
...
`r solbegin(show = params$SHOW_SOLS, toggle = params$TOGGLE)`
A plot of the residuals against the fitted values should not show any patterns or clumps. The values should appear as randomly scattered.
...
`r optbegin('Observations over time', FALSE, show = TRUE, toggle = params$TOGGLE)`
...
:::int
We performed a Durbin-Watson test against the null hypothesis of no correlation ($D-W = 1.80$, $p = 0.522$, two-sided). 
...
Note that the distribution of the residuals has zero mean. We don't see any significant departures from normality from the histogram, but it’s difficult to judge normality from histograms with so few observations.
...
A more formal test for normality is the Shapiro-Wilks test against the null hypothesis that the sample comes from a normal distribution:
```{r}
...
We can make all residuals positive by discarding the sign (take absolute value), and then take the square root to make them closer to each other.
A plot of $\sqrt{|\text{Standardized residuals}|}$ against the fitted values is shown below:
```{r}
plot(mdl_perf, which = 3)
```
The plot above has the points much closer to each other, and all above 0. The spread of the two groups appears pretty similar, and the line connecting the centre of the two clouds seems to be flat (as it should be if the spread was constant).


We can also resort to a more formal statistical test against the null hypothesis of equal variance (homoschedasticity). 
...
:::int
The spread of the standardized residuals appears to be constant as the fitted value varies.
