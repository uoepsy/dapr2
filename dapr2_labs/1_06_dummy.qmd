...
We can fit our multiple regression model using the `lm()` function. For a recap, see the [statistical models flashcards](https://uoepsy.github.io/dapr2/2425/labs/1_b2_reading.html#statistical-models). 

Recall that `R` computes the dummy variables for us. Thus, each row in the `summary()` output of the model will correspond to one of the estimated $\beta$'s in the equation above. For a more in-depth recap, see the [numeric outcomes & categorical predictors flashcards](https://uoepsy.github.io/dapr2/2425/labs/1_b2_reading.html#numeric-outcomes-categorical-predictors)
