...
The estimate corresponding to (Intercept) contains $\hat \beta_0 = \hat \mu_1 = 22.14$. The estimated average spending for those having no music playing in the background is approximately £22.14.
...
| Coefficient | Estimate | Corresponds to |
|:-:|:-:|:-:|
| (Intercept)   |  24.1741 | $\beta_0 = \frac{\mu_1 + \mu_2 + \mu_3}{3} = \mu$
| musicNone     | -2.0328  | $\beta_1 = \mu_1 - \mu$ |
| musicPop      | -2.2752  | $\beta_2 = \mu_2 - \mu$ |

The estimate corresponding to `(Intercept)` contains $\beta_0 = \mu = 22.7382$. This value represents the global mean of the data. The estimated average spending for customers across background music conditions is approximately £22.74.

The next estimate corresponds to `music1` and is $\hat \beta_1 = -0.5968$. The difference in mean spending between `None` and the global mean is estimated to be $-0.2424$. In other words, people with no music playing in the background seem to spend approximately £0.60 less than average. 

The estimate corresponding to `music2` is $\hat \beta_1 = -0.8392$. The difference in mean spending between `Pop` and the global mean is estimated to be $-0.8392$. In other words, customers with Pop music playing in the background seem to spend approximately £0.84 less than average.

The estimate for `music3`, representing the difference of “Classical” to the global mean is *not shown* by `summary()`. Because of the side-constraint, we know it must be $\beta_3 = -(\beta_1 + \beta_2)$. The difference in mean spending between `Classical` and the global mean is estimated to be $-(-0.5968 + -0.8392) = 1.436$. In other words, customers with Classical music playing in the background seem to spend approximately £1.44 more than average.
...
m_rg <- lm(amount ~ music, data = rest_spend)
...
#add predicted values from our two models - m_rg & m_stz - values are the same
music_groups %>%
  mutate(
    pred_dummy = predict(m_rg, newdata = .),
