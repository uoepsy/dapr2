...
Open the help page of the function `, and look at the examples.
Using `fct_relevel()`, reorder the levels of the factor `music` so that your reference group is "None".

:::{.callout-tip appearance="simple" collapse="true"}

### Hint

After you reorder the levels, you should end up with the following coding of group means:
...
Re-fit the linear model, and inspect the model `summary()` once more.
...
First, re-run the model and check the summary output: 
...
The displayed coefficients have changed as now the (Intercept) represents the new reference group ($\mu_1$), which is "None".
...
`r qbegin(8)`
Set the sum to zero constraint for the factor `music`.
...
The first level is "None", while the second level is "Classical".
...
The estimate for `music3`, representing the difference of "Pop" to the global mean is not shown. Because of the side-constraint, we know it must be $\beta_3 = \beta_0 - (\beta_1 + \beta_2) = - (-0.5968 + 1.4360) =  -0.8392$.
