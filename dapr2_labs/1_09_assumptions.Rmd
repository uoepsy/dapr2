...
(p1 | p3 ) + plot_annotation(title = "Non-constant variance (ncvTest p<.05)")
```

```{r ncv2, echo=FALSE, fig.cap="Constant variance for numeric and categorical x", fig.width = 8, out.width = '90%'}
(p2 | p4) + plot_annotation(title = "Constant variance (ncvTest p>.05)")
```
...
Furthermore, we performed a Shapiro-Wilks test against the null hypothesis that the residuals come from a normal population. The test-statistic $W = 0.96$ leads to a p-value of .35, and thus we do not have sufficient evidence to reject the null hypothesis at the 5% significance level.
:::
...
To get a single figure made up of 2 by 2 panels, you can use the command `par(mfrow = c(2,2))`. Then create the plot. Then you need to go back to a single figure made up by a single panel with the command `par(mfrow = c(1,1))`.
