...
:::green
Be sure to check the [**solutions to last week's exercises**](07_mlr_int.html).<br>You can still ask any questions about previous weeks' materials if things aren't clear!
:::
...
1. DO WE WANT TO INCLUDE THIS? Understand how to use transformations when the model violates assumptions.
...
:::green
In this lab, we will (1) check the assumptions of two models - one simple linear model, and one with multiple predictors; assess whether these models meet the assumptions outlined above; XXXXXX; and XXXXXXXX. We will be working with two different datasets that you have used in previous labs: `riverview` and `wellbeing`.
:::
...
$$ 
Model 1:\text{Income} = \beta_0 + \beta_1 \cdot \text{Education} + \epsilon
$$

$$
Model 2:\text{Wellbeing} = \beta_0 + \beta_1 \cdot \text{Outdoor Time} + \beta_2 \cdot \text{Social Interactions} + \epsilon
$$
...
wb_mdl1 <- lm(wellbeing ~ outdoor_time + social_int, data = mwdata) #wellbeing model
```
...
In simple linear regression (SLR) with only one explanatory variable, we could assess linearity through a simple scatterplot of the outcome variable against the explanatory. This would allow us to check if the errors have a mean of zero. That is, there is no pattern left in the residuals as the systematic trend has entirely been captured by the linear model. If this assumption was met, the residuals would appear to be randomly scattered around zero, hence showingno sign of violation of the zero-mean assumption.
...
```{r}
plot(rv_mdl1, 1) #here what we need to see is that the red line is approximately horizontal at zero
```
...
:::frame
The equal variances assumption is that the error variance $\sigma^2$ is constant across values of the predictor(s) $x_1$, ... $x_k$, and across values of the fitted values $\hat y$. This sometimes gets termed "Constant" vs "Non-constant" variance. Figures \@ref(fig:ncv1) & \@ref(fig:ncv2) shows what these look like visually. 
...
`r qbegin(3.1)`
Check if the fitted model `rv_mdl1` satisfies the equal variance assumption. Use `plot` to  `residualPlots()` to plot residuals against the predictor, and use `ncvTest()` to perform a test against the alternative hypothesis that the error variance changes with the level of the fitted value.  
...
We can also resort to the more formal Breusch-Pagan test:
```{r}
...
We performed a Breusch-Pagan test against the null hypothesis of constant variance ($\chi^2(1) = 0.01, p = .93$).
At the 5\% significance level, the large p-value (.93) indicates that the sample results do not provide sufficient evidence to reject the null hypothesis that the errors have constant variance.
:::

`r solend()`

`r qbegin(3.2)`
Check if the fitted model `wb_mdl1` satisfies the equal variance assumption using `residualPlots` and `ncvTest`. As above,remember to summarise whether or not you consider the assumption to have been met by making  reference to plots and/or formal tests where available.
`r qend()`
...
Visual inspection of suggested little sign of non-constant variance, with the Breusch-Pagan test failing to reject the null that error varance does not change across the fitted values ($\chi^2(1)=0.002$, $p = .97$).
:::
...
<br>
You can either do this quickly by giving the `plot()` function your model. Be sure to specify which plot you want to return - e.g., `plot(wb_mdl1, which = ???)`
