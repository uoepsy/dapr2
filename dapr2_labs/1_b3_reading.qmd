...
Interaction models allow us to model the idea of “the association between $x_1$ and $y$ changes depending on the level of $x_2$” by including a product (multiplication) term between the two predictors.
...
`r optbegin("Examples & Visualisations", olabel=FALSE,toggle=params$TOGGLE)`

```{r}
library(palmerpenguins)
```

For our examples, we are going to use the *palmer penguins* dataset^[Horst, A. M., Hill, A. P., & Gorman, K. B. (2020). palmerpenguins: Palmer Archipelago (Antarctica) penguin data. R package version 0.1.0. https://allisonhorst.github.io/palmerpenguins/ doi: 10.5281/zenodo.3960218].
...
```{r}
mdl_nc <- lm(body_mass_g ~ flipper_length_mm * species, data = penguins)
summary(mdl_nc)
```
...
```{r}
penguins %>%
  ggplot(., aes(x = flipper_length_mm, y = body_mass_g, colour = factor(bill_length_mm))) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "Flipper Length (in mm)", y = "Body Mass (in g)")
```
...
```{r}
#scatterplot3d(penguins$flipper_length_mm, penguins$body_mass_g, penguins$bill_length_mm, pch = 19, color = "blue", main = "3D Scatterplot")
```
...
```{r}
mdl_nn <- lm(body_mass_g ~ flipper_length_mm * bill_length_mm, data = penguins)
summary(mdl_nn)
```
...
`r optbegin("Example", olabel=FALSE,toggle=params$TOGGLE)`
