...
$$
Wellbeing = \beta_0 + \beta_1 \cdot Routine + \beta_2 \cdot OutdoorTime + \epsilon
$$
...
We can visualise the model $\widehat{Wellbeing} = \hat \beta_0 + \hat \beta_1 \cdot Routine + \hat \beta_2 \cdot OutdoorTime$ as two lines.  
...
| $\hat \beta_1$       | `routineRoutine`          | `r round(coef(mdl2)[2],2)` |
| $\hat \beta_2$       | `outdoor_time`            | `r round(coef(mdl2)[3],2)` |
...
+ A = $\hat \beta_2$ = `routineRoutine` coefficient = `r round(coef(mdl2)[2],2)`
+ B = $\hat \beta_0$ = `(Intercept)` coefficient  = `r round(coef(mdl2)[1],2)`
+ C = $\hat \beta_1$ = `outdoor_time` coefficient = `r round(coef(mdl2)[3],2)` 
...
The researchers have decided that they would prefer 'routine' to be considered the reference level for the "routine" variable. Apply this change to the variable and re-run your model.
...
```{r}
#ensure routine is coded as a factor
mwdata$routine  <- as_factor(mwdata$routine)

#re-order so that routine is baseline/reference level
mwdata$routine <- relevel(mwdata$routine, 'Routine')
...
You should now see that our routine variable is now showing  `routineNo Routine` instead of `routineRoutine`. This means that 1 is now for "No Routine" and 0 for "Routine".
...
See [Week 1](https://uoepsy.github.io/dapr2/2223/labs/1_01_function.html) for further details and to revise these concepts further.
...
See [Week 2](https://uoepsy.github.io/dapr2/2223/labs/1_02_slr.html) for further details, examples, and to revise these concepts further.
...
See [Week 3](https://uoepsy.github.io/dapr2/2223/labs/1_03_mlr.html) for further details, examples, and to revise these concepts further.
...
**Formula**: $$SS_{Total} = \sum_{i=1}^{n}(y_i - \bar{y})^2$$

**In words**: Squared distance of each data point from the mean of $y$.

**Description**: How much variation there is in the DV - a measure that captures the spread of the observed variables around the mean. The Total Sum of Squares quantifies the total variation in the sample / dataset. 

#### __Residual Sum of Squares__

**Formula**: $$SS_{Residual} = \sum_{i=1}^{n}(y_i - \hat{y}_i)^2$$

**In words**: Squared distance of each point from the predicted value.

**Description**: How much of the variation in the DV the model did not explain - a measure that captures the unexplained variation in your regression model. Lower residual sum of squares suggests that your model fits the data well, and higher suggests that the model poorly explains the data (in other words, the lower the value, the better the regression model). If the value was zero here, it would suggest the model fits perfectly with no error.

#### __Model Sum of Squares__ 

**Formula**: $$SS_{Model} = \sum_{i=1}^{n}(\hat{y}_i - \bar{y})^2$$

Can also be derived from:
$$SS_{Model} = SS_{Total} - SS_{Residual}$$
**In words**: The deviance of the predicted scores from the mean of $y$.

**Description**: How much of the variation in the DV your model explained - like a measure that captures how well the regression line fits your data.
...
See [Week 4](https://uoepsy.github.io/dapr2/2223/labs/1_04_model_fit.html) for further details, examples, and to revise these concepts further.
