...
2. Have completed [Week 1](https://uoepsy.github.io/dapr2/2324/labs/1_01_slr.html) and [Week 2](https://uoepsy.github.io/dapr2/2324/labs/1_02_mlr.html) lab exercises
...
$$
where $t^*$ denotes the critical value chosen from t-distribution with $n-k-1 = 200 - 2 - 1 = 200 - 3$ degrees of freedom for a desired $\alpha$ level of confidence. 
...
From comparing either the `summary()` or rounded output, we can see that the estimates are the same under both approaches. That means you can use either to standardize the variables in your model. 
...
Examine the 'Coefficients' section of the `summary()` output from the standardized and unstandardized models - what do you notice?
...
We can see that the slope estimates from the standardized model are equivalent to the part correlation coefficients. 
