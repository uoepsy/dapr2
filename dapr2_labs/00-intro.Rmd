---
title: "Intro to R, RStudio, and data in R"
...
__B:__ Use R and RStudio online via a RStudioCloud in a web browser (for people using chromebooks).  

:::red
**If you have any problems installing RStudio, please try to use RStudioCloud (option B)**
:::

`r optbegin("Option A: Installing R and RStudio (recommended)",olabel=FALSE)`
...
`r optbegin("Option B: RStudio Cloud (for chromebook users)",olabel=FALSE)`
...
To assign a sequence ("vector") of values to R, we combine the values using `c()`. 
...
:::yellow
__Functions__ perform specific operations in computer programming.  
  
They can have __inputs__ and __outputs__. For example, `dim()` takes some data you have stored in R as it's __input__, and gives the dimensions of the data as its __output__. 
:::
