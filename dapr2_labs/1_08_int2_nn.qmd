...
+ $\beta_1$ = `zscs` = `r round(coef(dass_mdl)[2],2)`
    - The simple slope of SCS scores when Neuroticism is X. 
    - For those with X Neuroticism, DASS-21 scores decreased by 1.57 standard deviations for every 1 standard deviation increase in SCS scores, 
    
+ $\beta_2$ = `zn` = `r round(coef(dass_mdl)[3],2)`
    - The simple slope of Neuroticism  when number of social interactions is 0.
