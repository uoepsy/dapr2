...
- The function says that the $y$ value is obtained as a transformation of the $x$ value: five times $x$, plus two.
<br>
...
Sometimes, we can read predicted values from the graph of the functional relationship.
...
`r optbegin('Data: handheight.csv', FALSE, show = params$SHOW_SOLS, toggle = params$TOGGLE)`
**Download link**
...
From the scatterplot in Figure \@ref(fig:handheight-scatterplot) we do not notice any extreme observations or points that do not fit with the rest of the data.
