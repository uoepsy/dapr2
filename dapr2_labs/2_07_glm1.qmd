...
- When considering the scale of your BAC and age variables, review the [data transformation flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#data-transformations). 
...
*See the [numeric outcomes & categorical predictors flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#numeric-outcomes-categorical-predictors), including the [example flashcard > specifying reference levels](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#example-2).  
:::
...
Much of the information required can be found in the [Study Overview] codebook. The [statistical models flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#statistical-models), especially the [logistic models flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#logistic-models) may also be useful to refer to. For a more in-depth overview and example, see the [logistic models > example flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#example-3).
...
Review the many ways to numerically and visually explore your data by reading over the [data exploration flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b2_reading.html#data-exploration).

For examples, see flashcards on [descriptives statistics tables](https://uoepsy.github.io/dapr2/2526/labs/1_b2_reading.html#descriptives) and [data visualisation > bivariate associations - examples flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b2_reading.html#visual-exploration), paying particular attention to the type of data that you're working with. The [logistic models > example flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#example-3) may also be helpful to refer to.
...
door_plt1 <- drunkdoor |>
    ggplot(aes(x=as_factor(notice), fill=as_factor(notice))) +
    geom_bar() +
...
door_plt2 <- drunkdoor |>
    ggplot(aes(x=age, fill=as_factor(notice))) +
    geom_density() +
...
door_plt3 <- drunkdoor |>
    ggplot(aes(x=bac, fill=as_factor(notice))) +
    geom_density() +
...
door_plt4 <- drunkdoor |>
    ggplot(aes(x=condition, fill=as_factor(notice))) +
...
```{r}
#| label: fig-drunk5
#| fig-cap: "Association between Notice, Age, BAC, and Perceptual Load Condition"

door_plt5 <- drunkdoor |> 
    ggplot(aes(x = age, y = notice)) +
    ylab("p(notice)") +
    geom_jitter(aes(colour = bac), size = 2) +
...
For an overview of how to fit a binary logistic regression model using the `glm()` function, see the [binary logistic regression flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#logistic-models).   
  
For an example, review the [logistic models > example flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#example-3).  
...
Consider whether or not your models are nested. The [model comparisons > logistic models flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#logistic-models-2) may also be helpful to revisit.
...
For an overview of how probability, odds, and log-odds are related, review the [probability, odds, and log-odds flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#logistic-models).  

If you need help interpreting coefficients, review the [interpretation of coefficients flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#logistic-models).  
...
Firstly, produce plots of the predicted probabilities of noticing the swap as a function of (a) age, (b) BAC, and (c) condition.

Next, plot the predicted model estimates. 

:::{.callout-tip appearance="simple" collapse="true"}

### Hint

For the predicted probabilities, you will need to use `effect()` from the **effects** package, as well as `ggplot()` from the **tidyverse** package. Predicted probabilities for categorical IVs need to be visualised using point estimates with confidence intervals (i.e., `geom_pointrange()`), whereas continuous IVs need to be visualised using smoothed lines with confidence interval bands (i.e., `geom_line()` and `geom_ribbon()`).     
            
For the predicted model estimates, you will need to use `plot_model()` from the **sjPlot** package.                 
            
For a quick overview and example, review the [logistic models > example > model visualisation flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#example-3).               
...
```{r}
pp_age <- effect(term = "age_mc", mod = changeblind_mdl, xlevels = 20) |>
  as.data.frame() |>
  ggplot(aes(x=age_mc,y=fit,ymin=lower,ymax=upper))+
  geom_line()+
  geom_ribbon(alpha=.3) + 
  labs(title = ("A"))

pp_bac <- effect(term = "bac100", mod = changeblind_mdl, xlevels = 20) |>
  as.data.frame() |>
  ggplot(aes(x=bac100,y=fit,ymin=lower,ymax=upper))+
  geom_line()+
  geom_ribbon(alpha=.3) + 
  labs(title = ("B"))

pp_condition <- effect(term = "condition", mod = changeblind_mdl) |>
  as.data.frame() |>
  ggplot(aes(x=condition,y=fit,ymin=lower,ymax=upper))+
  geom_pointrange() + 
  labs(title = ("C"))

(pp_age + pp_bac) / (pp_condition)
```
...
Use `tab_model()` from the __sjPlot__ package. For a quick guide, review the [tables flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#tables).
...
For an example of coefficient interpretation, review the [interpretation of coefficients flashcard](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#logistic-models). For a more in-depth overview and example, see the [logistic models > example flashcards](https://uoepsy.github.io/dapr2/2526/labs/1_b4_reading.html#example-3).  
