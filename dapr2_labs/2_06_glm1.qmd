...
2. For your visualisations, you will need to specify `as_factor()` when plotting the notice variable
...
door_plt4 <- ggplot(data = drunkdoor, aes(x=condition, fill=as_factor(notice))) +
  ...
  labs(fill = "Noticed Swap (0 = No, 1 = Yes)", xlab = "Condition")
door_plt4
```
...
Hopefully you can see that the model predicted value for someone aged 30 is approximately 1.15. 
...
At the 5% significance level, the addition of information about the participants’ age, BAC, and perceptual load resulted in a significant decrease in model deviance (65). The associated $p$-value from a chi-squared distribution with 3 degrees of freedom results was $p$ < .001.
...
+ $\beta_0$ = `(Intercept)` = `r round(exp(coef(changeblind_mdl))[1],2)`
    - The odds of noticing a mid-conversation person-switch for someone of average age, with 0 BAC, with Low perceptual load was 0.60. 
