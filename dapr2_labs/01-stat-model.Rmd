...
$$
\texttt{data = deterministic model}
$$
...
$$
\begin{aligned}
\texttt{data} &= \texttt{deterministic model + random error} \\
y &= f(x) + \epsilon
\end{aligned}
$$
...
We will investigate each step in turn using the [perfection.csv](#dc-perfection) data. Read the data description in the [data codebook](#dc-perfection).
...
Recalling that $\mu_i$ denotes the population mean response in the $i$th group, this can also be reformulated in the familiar DATA = DETERMINISTIC MODEL + RANDOM ERROR equation as follows:
$$
\begin{split}
