...
Generate the model summary for `sc_mdl` and write a sentence to interpret each coefficient.

For the interaction term, challenge yourself to write both equivalent interpretations.
...
Here's a plot of the simple slopes from `sc_mdl`; that is, the lines associating `zscs` with `dass` for specific values of `zn` (namely the mean, the mean + 1 SD, and the mean – 1 SD).
