...
From @fig-marg_dist, we can see that wellbeing $(M = `r round(mean(mwdata$wellbeing), 2)`, SD = `r round(sd(mwdata$wellbeing), 2)`)$, social interactions $(M = `r round(mean(mwdata$social_int), 2)`, SD = `r round(sd(mwdata$social_int), 2)`)$, and outdoor time $(M = `r round(mean(mwdata$outdoor_time), 2)`, SD = `r round(sd(mwdata$outdoor_time), 2)`)$ followed unimodal distributions. There were weak, positive, linear associations between WEMWBS scores and the weekly number of social interactions $(r = .24)$, and between WEMWBS scores and outdoor time $(r = .25)$ for the participants in the sample.
