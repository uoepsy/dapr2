...
1. Understand how to write-up and provide interpretation of a 4x2 factorial ANOVA
...
To investigate whether study condition (No, Minimal, Moderate, Extensive) and note-taking medium (Longhand, Laptop) interacted to influence test scores, a two-way ANOVA model was used. Effects were considered statistically significant at $\alpha = 0.05$. Using dummy coding, the following model specification was used:

$$
\begin{align}
\text{Test Score} &= \beta_0  \\
      &+ \beta_1 \cdot M_\text{Laptop} \\  
      &+ \beta_2 \cdot S_\text{Minimal} \\ 
      &+ \beta_3 \cdot S_\text{Moderate} \\
      &+ \beta_4 \cdot S_\text{Extensive} \\
      &+ \beta_5 \cdot  (M_\text{Laptop} \cdot  S_\text{Minimal})  \\
      &+ \beta_6 \cdot  (M_\text{Laptop} \cdot  S_\text{Moderate})  \\
      &+ \beta_7 \cdot  (M_\text{Laptop} \cdot  S_\text{Extensive})  \\
      &+ \epsilon  
\end{align}
$$
...
To address RQ2 and explore differences between each pair of levels of each factor to determine which conditions significantly differed from each other, we will conduct a series of pairwise comparisons. Since we are interested in all pairwise comparisons of means, we will apply a Tukey correction. 
...
See [S1 Week 9 Lab](https://uoepsy.github.io/dapr2/2324/labs/1_08_assump_diag.html), [S1 Week 9 Lectures](https://uoepsy.github.io/dapr2/2324/lectures/dapr2_08_assumptions_diagnostics.html#1), [S2 Week 1 Lectures]([S1 Week 9 Lectures](https://uoepsy.github.io/dapr2/2324/lectures/dapr2_11_interactions1.html#1) for further details, examples, and to revise these concepts further.
