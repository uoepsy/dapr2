...
_**Hint**: Use the `geom_point()` function._
`r qend()`
...
The estimates of the model parameters are computed from the available sample data and are denoted:

- estimated intercept $\hat \beta_0$;
- estimated slope $\hat \beta_1$;
- estimated standard deviation of the errors $\hat \sigma$.

Once the model parameters have been estimated --- that is, the line has been *fitted* to the sample data --- you are able to use the model to estimate the average response $y$ at a given value of the explanatory variable $x$.

The *fitted model*, returning the predicted response is:
$$
...
`r qbegin(6)`
Fit the linear model in R using the function `lm()` and assign the output to an object called `mdl`.

The syntax is: `lm( <response variable> ~ 1 + <explanatory variable>, data = <dataframe> )`

_**Hint:** The fitted model $\hat y = \hat \beta_0 + \hat \beta_1 \cdot x$ can be equivalently written as $\hat y = \hat \beta_0 \cdot 1+ \hat \beta_1 \cdot x$._

_When we specify the linear model in R, we include after the tilde `~` the variables that appear to the right of the $\hat \beta$s, explaining why the 1 is included._
`r qend()`
...
`r qbegin(7)`
a. Explore the following equivalent ways to obtain the estimated intercept and slope from the fitted model `mdl`:
...
b. Explore the following equivalent ways to obtain the estimated sigma from the fitted model `mdl`:
...
`r solbegin()`
a. To obtain the estimated coefficients you can either:
...
b. The estimated standard deviation of the errors can be equivalently obtained by:

- typing `sigma(mdl)`;
- looking at the "Residual standard error" entry of the `summary(mdl)` output.

**Note:** As `help(sigma)` says, the term "Residual standard error" is a misnomer but it's hard to get rid of this bad practice as it has been used in too many books which show R output.
...
- `predict(<FITTED MODEL>)`: returns the fitted values for the units in the sample;
- `predict(<FITTED MODEL>, newdata = <NEW DATA>)`: returns the predictions for the new predictor values provided in `<NEW DATA>`.
...
To assess how well the model is capturing the variability of the response, we can split the total variability in the response $y$ as the sum of the variability explained by the model (the fitted regression line) plus the variability that is left unexplained in the residuals/errors.
...
_**Hint:** The question asks what is the value of $R^2 = SSModel / SSTotal$ for this model?_
`r qend()`
...
$$
t = \frac{\hat \beta_1}{SE} = \frac{\hat \beta_1}{\hat \sigma / \sqrt{\sum (x_i - \bar x)^2}}
$$
