...
The plot above has the majority of the points close to each other, and all above 0. The line seems to be relatively flat (as it should be if the spread was constant), but there is some deviation from this (e.g., observations 16, 78, and 109 appear quite far away from the cluster of points). We could hesitantly conclude at this point that the data met the assumption of equal variance.

:::

::: {.callout-important icon=false appearance="minimal"}

Partial residual plots show no clear non-linear trends between residuals and predictors, hence there is little sign of non-constant variance. The data met the assumption of equal variance as the spread of the standardized residuals appears to be constant (for the most part) as the fitted values vary.
...
The QQplot indicated that the residuals follow close to a normal distribution, as the points follow linear pattern and there is no substantial skew or departure from normality. There is some evidence of heavier tails, and we may want to examine some observations more closely (i.e., 16, 78, 109).
...
There are `r sum(abs(mdl_diagnost$studres) > 2)` observations that have been identified as potential outliers.
...
There are `r sum(mdl_diagnost$hats > (2*0.015))` observations that have high leverage (> $2 \bar h$).  
...
There are `r sum(mdl_diagnost$cooksd > 0.020)` observations that have a high influence on our model estimates
...
It looks like a few observations may be having quite a high influence here.  
...
There are lots of different options available, and there is no one right answer. 
