...
:::green
Be sure to check the [**solutions to last week's exercises**](05_mlr_int.html).<br>You can still ask any questions about previous weeks' materials if things aren't clear!
:::
...
To investigate how the effect of social comparison on symptoms of depression, anxiety and stress may vary depending on level of neuroticism, we will need to fit a multiple regression model with an interaction term. Before we think about fitting our model, it is important that we understand the data available - how many participants? What type (or class) of data will we be working with? What was the measurement scale? How were scales scored? Look at the social comparison study data codebook below. 
