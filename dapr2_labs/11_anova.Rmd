...
$$
y = \beta_0 + \beta_1 \ \text{EffectLevel1} + \beta_2 \ \text{EffectLevel2} + \epsilon \\
$$
...
```
rest_spend$type <- contr.sum
mdl <- lm(amount ~ 1 + type, data = rest_spend)
# or: mdl <- lm(amount ~ type, data = rest_spend)
```
...
- $\beta_0$ is estimated overall mean
- $\beta_1$ is the effect of Classical Music
- $\beta_2$ is the effect of No Music
- **NOTE**: The effect of Pop Music is not shown by `summary` as it is found via the side-constraint as `-(beta1 + beta2)`!
...
y 
= \beta_0 
+ \beta_1 \ \text{IsTypeNoMusic} 
+ \beta_2 \ \text{IsTypePopMusic} 
+\epsilon
$$
...
```
rest_spend$type <- contr.treatment
mdl <- lm(amount ~ 1 + type, data = rest_spend)
# or: mdl <- lm(amount ~ type, data = rest_spend)
```
...
- $\beta_0$ is the mean of the base level (level 1)
- $\beta_1$ is the effect of No Music
- $\beta_2$ is the effect of Pop Music
