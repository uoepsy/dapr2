...
`r qbegin(2)`
Create a frequency table showing the number of people which were assigned to each type of background music.
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
...
If we order the groups by alphabetical ordering, we can represent the data as follows:
...
- $i = 1, 2, 3$ counts the type of music played - i.e. the group
- $j = 1, 2, ..., 120$ counts the observation within a group
...
You can think of $\beta_0$ as a baseline value, and the $\beta_1, \beta_2, \beta_3$ as group effects.
...
Before continuing, however, let us define the overall group mean as $\mu = \frac{\mu_1 + \mu_2 + \mu_3}{3}$.
...
Depending on the side-constraint, the interpretation of $\beta_0$ and $\beta_1$ will change.
...
The models presented before are just multiple linear regression models where the $x$s are binary 0/1 variables.
...
is equivalent to the following linear models

## Sum-to-zero case

$$
y_{i} = \beta_0 + \beta_2 \cdot d_{i,1} + \beta_3 \cdot d_{i,2} + \epsilon_{i}
$$
...
\text{Intercept = Overall mean}
$$

## Reference-group case

$$
y_{i} = \beta_0 + \beta_2 \cdot d_{i,2} + \beta_3 \cdot d_{i,3} + \epsilon_{i}
$$
