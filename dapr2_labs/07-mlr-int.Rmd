...
Response: Quantitative<br />Predictors: ... * ...
</h3>
</center>
<br />
...
2. Interpret the meaning of an quantitative $\times$ quantitative interaction, and a quantitative $\times$ qualitative interaction.  
...
```{r}
library(tidyverse)
mwdata <- read_csv("data/wellbeing.csv")
...
[Download the data here](https://raw.githubusercontent.com/uoepsy/dapr2/master/dapr2_labs/data/wellbeing_rural.csv)
...
```{r echo=FALSE}
read_csv('data/wellbeing_rural.csv') %>% head %>%
...
In our model: $\text{wellbeing} = \beta_0 + \beta_1\cdot\text{social-interactions} + \beta_2\cdot\text{isRural} + \beta_3\cdot\text{social-interactions}\cdot\text{isRural} + \epsilon$, we have a quantitative*qualitative interaction.  
...
We will now look at a multiple regression model with an interaction betweeen two quantitative explanatory variables.

> **Research question**   
> Research has identified an association between an individual's perception of their social rank and symptoms of depression, anxiety and stress. We are interested in the individual differences in this relationship.  
Specifically: 
> 
> + Is there an interaction between the neuroticism and social comparison in predicting symptoms of depression, anxiety and stress?
> 
...
[Download the data here](https://raw.githubusercontent.com/uoepsy/dapr2/master/dapr2_labs/data/scs_study.csv)
...
```{r echo=FALSE}
read_csv('data/scs_study.csv') %>% head %>%
  knitr::kable(., align='c') %>% kableExtra::kable_styling(full_width = FALSE)
```
...
`r qbegin(7)`
Specify the model you plan to fit (e.g., $\text{??} = \beta_0 + \beta_1 \cdot \text{??} + .... + \epsilon$)
`r qend()`
`r solbegin(show=SHOW_SOLS)`
$$
...
Cutting one of the explanatory variables up into groups essentially turns a quantitative variable into a categorical one. We did this just to make it easier to visualise how a relationship changes across the values of another variable, because we can imagine a separate line for the relationship between SCS and DASS-21 scores for each of the groups of neuroticism. However, in grouping a quantitative variable like this we lose information. Neuroticism is measured on a continuous scale, and we want to capture how the relationship between SCS and DASS-21 changes across that continuum (rather than cutting it into chunks).   
