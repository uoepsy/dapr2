...
salary3 <- tibble(
  ...
  location = factor(location, labels = c("London", "Edinburgh")),
  serv = serv
)
...
+ Suppose our company also had two sites, London and Edinburgh, and we wanted to see if the salaries across department differed depending on location.
...
colnames(eg1) <- c("London","Edinburgh")
...
```{r,, echo=FALSE}
dat1 <- tibble(
  location = c("London", "London", "Edinburgh", "Edinburgh"),
...
colnames(eg2) <- c("London","Edinburgh")
...
colnames(eg3) <- c("London","Edinburgh")
...
+ The difference in salary between Accounts in London and Edinburgh
...
+ The difference between salary in Accounts and Store managers between London and Edinburgh
...
colnames(ex1_means) <- c("London","Edinburgh")
...
+ The difference in salary between Accounts in London and Edinburgh is £1,980. The salary is lower in Edinburgh. 
...
+ The difference between salary for Accounts and Store managers between London and Edinburgh, differs by £8,420. The difference is greater in Edinburgh than in London.
...
+ Consider our salary example:
  + We could ask does the change in difference in the effect of years of service across departments, differ dependent on whether you are based in London or Edinburgh.
  
+ Note we have three predictors here:
  + Years of service (continuous)
  + department (binary; Store Manager, Accounts)
  + location (binary; London, Edinburgh)
