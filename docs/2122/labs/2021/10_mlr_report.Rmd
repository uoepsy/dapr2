...
`r optbegin("Research question and analysis. Click the plus to expand &#8594;", olabel=FALSE)`
> **Research question**   
...
`r solbegin("SPOILERS ALERT", show=TRUE, toggle=params$TOGGLE)`
:::frame
...
pairs.panels(scs_study %>% select(-zscs))
```
^[Created with the `pairs.panels()` function from the **psych** package if you're interested.]
