...
+ Recall when we have a linear model with multiple predictors, we interpret the $\beta_j$ as the effect "hold all other variables constant".
...
+ When we include a higher-order term/interaction in our models ( $\beta_3$ ), it is critical we include the main effects ( $\beta_1$ , $\beta_2$ ).
  ...
  + Conside dropping $\beta_2$ in our example.
  + Common intercept.
